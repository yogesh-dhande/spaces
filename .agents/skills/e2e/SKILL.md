...
- **`app`**: window cycling, focus, sidebar targets, and the Editor. Scenarios include `smoke`/`window-cycle`/`window-cycle-small` for the compact profile, `full` for everything, and `code-pane`, `code-pane-streaming`, `code-pane-churn`, `code-pane-soak` for the Editor. Desktop-control heavy: see §2.
- **`terminal`**: daemon and CLI behaviour with no app. Scenarios include `cli`, `daemon-idle-shutdown`, `daemon-exec-handoff`, `daemon-signal-shutdown`, `session-restore`, `edit-shortcuts`, `mouse-reporting-scroll`, `ended-session-scroll`, `selection-scroll`, `agent-orchestration`, `workspace-process-terminal`, plus the profiling scenarios (`built-in-terminal-profile`, `workspace-terminal-open`, `spaces-terminal-hotkeys`, `spaces-terminal-palette`, `stress`, `soak`, `device-api-profile`) and the `mac-*-latency` set.
- **`mobile`**: simulator UI scenarios, including `takeover`, `codex`, `roundtrip`, `scrollback`, `two-session`, `ownership-guard`, and the `ios-*-latency` set.
- **`device-api`**: the target is a positional argument (`e2e.sh device-api latency-compare`): `local`, `remote`, `all` (local and remote, the default), `latency-compare`, `profile`. Everything but `local` needs the remote host.
...
7. **Remote lanes need `.env`.** `scripts/spaces-e2e-env.sh` sources the gitignored repo-root `.env`; a fresh worktree has none, so copy it in. Remote lanes deploy this worktree's daemon themselves: they build and upload it with `apps/macos/scripts/deploy_linux_spacesd_e2e.sh`, then install it into their own remote profile. `mobile-baseline --remote` does not, so deploy first with `scripts/dev-build-and-launch.sh` (without `--local`). Tailscale SSH returns exit 0 for everything, so never trust a remote exit code alone; check the output.
