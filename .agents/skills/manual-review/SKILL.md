...
- Anything the iOS client can reach → **Mac app + paired iOS simulator**. That is not just `apps/ios`: daemon and Device API changes (`spacesd`, `workspacecore`, `spacesdeviceapi`, `spacesdatabase`, `spacesterminalcore`, `spacesdevicecore`, `spacesclientcore` protocol changes) are mobile-reachable because iOS consumes the same daemon behavior.
...
4. Pair against the worktree's daemon: `<worktree>/apps/macos/.build/debug/spacese2e open-device-pairing-window` prints a JSON object; extract its `pairingLink` (the `spaces://pair?...` URL) and deliver that with `xcrun simctl openurl <udid> "<link>"`. The app raises its pairing confirmation sheet; give the user the JSON's `pairingCode` so they can compare. Windows are one-time and expire, so open a fresh one per attempt.
