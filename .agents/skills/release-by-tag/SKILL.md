...
6. Verify that the iOS build reached TestFlight. `iOS Release` signs against Apple Developer account state (stored development and distribution certificates that expire yearly, an App Store Connect API key, a provisioning profile Apple issues), so it fails with nothing wrong in the repository and no failing check on the release commit. Read the workflow's run history across the preceding release tags as well: a signing failure persists until the account is repaired, so a first failure and a standing outage are indistinguishable from a single run.
