...
APP="$repo_root/apps/macos/.build/debug/SpaceshipApp"
LOG_FILE="/tmp/SpaceshipApp.log"
...
if ! kill -0 "$app_pid" 2>/dev/null; then
  echo "Spaceship exited immediately; last log lines:"
  tail -n 80 "$LOG_FILE" || true
  exit 1
fi

echo "Spaceship relaunched (pid $app_pid)"
