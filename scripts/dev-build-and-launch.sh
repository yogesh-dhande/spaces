...
if [ "$POST_LAUNCH_MONITOR_SECONDS" -gt 0 ]; then
  ...
  for _ in $(seq 1 "$POST_LAUNCH_MONITOR_SECONDS"); do
    if ! kill -0 "$app_pid" 2>/dev/null; then
      echo "Muxy exited during post-launch monitoring window."
      print_failure_diagnostics
      exit 1
    fi
    sleep 1
  done
  echo "Muxy stayed running for ${POST_LAUNCH_MONITOR_SECONDS}s after launch."
fi
