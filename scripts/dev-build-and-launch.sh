...
APP="$repo_root/apps/macos/.build/debug/spaceship-app"
LOG_FILE="/tmp/spaceship-app.log"
...
if ! kill -0 "$app_pid" 2>/dev/null; then
  echo "spaceship-app exited immediately; last log lines:"
  tail -n 80 "$LOG_FILE" || true
  exit 1
fi

echo "spaceship-app relaunched (pid $app_pid)"
