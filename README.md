...
- **Agent briefs**: each coding agent keeps a short markdown brief beside its terminal (status with an ETA, questions for you, its task list), written through the CLI or the MCP server and shown on the Mac and the iPhone. Read what an agent is doing without reading its transcript.
...
- **Keyboard shortcuts** — Next/Previous window cycling steps through a selectable scope: one workspace, everything in Alerts, every live agent, or every open session, any of them across devices. A leader chord in Spaces or the sidebar's cycling row picks the scope.
