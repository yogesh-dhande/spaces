...
## Docs
- Put product behavior as business logic in `docs/spec.md`: the non-standard, user-visible rules (what happens, when, and why), not UI placement or styling.
    - Update `docs/spec.md` when those rules change.
...
- User-facing docs are in `apps/web` which is published as a static website
    ...
    - Update the matching `apps/web/app/docs/<topic>/page.tsx` when user-visible behavior it describes changes.
...
- When fixing a bug, reproduce it first by writing a failing test and/or using the real system, this worktree's `apps/macos/.build/debug/spaces` CLI, and/or database inspection when practical.
