...
- Choose a project folder with a path text field plus directory autocomplete rather than a native file picker. The field accepts `~` and absolute paths, surfaces directory suggestions from the selected device as the user types, and validates the path on commit so the same control works for local and remote devices.
...
- When more than one device is paired, the sidebar groups projects under a per-device section header; with a single device the header is omitted and the sidebar stays a flat project list. The header is a quiet, non-interactive caption (uppercased device name, no disclosure control and no selection), not another collapsible row, so projects keep the only disclosure column and the list reads as groups rather than two stacked tiers of rows. The caption carries the device's load state (loading or offline) inline as muted trailing text rather than as a separate row, and a device that is still loading or unreachable shows the caption alone with no children.
