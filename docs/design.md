...
- Workspace bands lead with a branch glyph (folder for non-git workspaces) and toggle collapse with the standard muted right-edge chevron. Row lifecycle actions (Run, Stop, Restart, New Terminal) live in long-press context menus rather than persistent trailing buttons.
