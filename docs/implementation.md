...
Automation runs execute in workspace-less terminal sessions, which the overview builder deliberately excludes from its workspace-scoped `sessions`. The normal session→pane resolution reads `overview.sessions`, so it cannot find them; opening a run's terminal instead synthesizes a `DeviceTerminalOpenRequest` from the run and its automation and opens it in a standalone panel window (automation sessions are never workspace panes). The seeded shell is display metadata only — the pane attaches to the daemon's existing session and streams its real render — so a running run shows a live pane and an ended one shows the read-only transcript replay through the same attach path used for any ended pane.
