...
- Device API transport uses two authentication layers: clients pin the daemon's self-signed TLS identity from pairing metadata, then include a per-client token issued during the short-lived pairing window. Tokens are stored only as hashes by the daemon and as secrets in the client Keychain. The local first-party client is bootstrapped (not paired through the window) and re-bootstraps on every sidebar reload; each bootstrap presents the token the client already holds, and the daemon keeps that token when it matches the stored pairing hash, minting a new one only on first launch or when no valid token is presented. Keeping the token stable is required because the mac GUI's live Device API connections (terminal state streams and per-request terminal controls) carry the token, so rotating it on a routine re-bootstrap would 401 those requests — dropping terminal input and surfacing "terminal session is no longer available".
...
- macOS remote-device pairing, terminal attach, browser forwarding, and editor opening require SSH to the same device. Remote pairing validates SSH with `BatchMode=yes` and `StrictHostKeyChecking=yes`, requires Darwin hosts to expose the DMG install markers, and probes Linux hosts for Ubuntu 24.04 on `x86_64` or `arm64`. A pairing or daemon-refresh flow runs several short SSH commands in sequence (config lookup, reachability check, install probe, `spaces pair`, and the Linux installer copy/run), so it opens one shared OpenSSH master connection (`ControlMaster`) up front and lets every later `ssh`/`scp` reuse it; only the master pays the TCP + key-exchange + auth cost, which keeps a jittery link from spiking a single cold connect past a command timeout and aborting the flow. The master is best-effort — its own cold connect is retried a bounded number of times, and if it cannot be established the commands fall back to direct connections — so it changes a flow's speed and resilience, never its outcome. When the Linux pairing command is missing or not responding, the client downloads the signed release manifest for the running app version, verifies it with the bundled remote-artifact Ed25519 public key, downloads and checks the matching archive, copies it over SSH, runs the included installer as the target user, and retries `~/.spaces/bin/spaces pair --json`. The Linux installer enables systemd user lingering before restarting the user service so Spaces stays available after the setup SSH command exits. Pairing then uses the Device API at the effective OpenSSH `HostName` and returned API port, so SSH aliases resolve to direct LAN, VPN, or Tailscale endpoints without using daemon-advertised interface metadata.
...
- Explicit pane close detaches process and coding-agent sessions while an ad hoc terminal's pane close uses the ad hoc session stop path. Terminal-toolbar stop uses the process, coding-agent, or ad hoc lifecycle path for the selected runtime; lifecycle actions never remove panes — the pane keeps showing the session's final render.
...
- Current-cycle resolution prefers built-in terminal identity before external probing. When the active surface is a built-in Spaces terminal, the hotkey path passes the session identity directly into the client cycle resolver so cycle resolution can skip the generic focused-window probe. Otherwise the client resolves the current target from the focused Chrome tab when Chrome is frontmost, then from the remembered navigation cursor.
