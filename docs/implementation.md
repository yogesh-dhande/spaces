...
An incompatible device is **fully blocked** — no mutations, no overview data rendered — while other paired devices stay fully functional. Restart is by RPC (`requestDaemonRestart`), which terminates the daemon gracefully so launchd `KeepAlive` or systemd `Restart=always` respawns it. A Mac picks up a staged `Spaces.app` update on that respawn. A Linux daemon respawns the same binary, so updating it is not a restart action; the block shows the version-pinned install one-liner for the user to run there.
