...
- Auto-install orchestration lives client-side because the app is the single decision-maker. `AgentHooksAutoInstaller` (`spacesclientcore`) reads the per-device recorded set from `ClientSettingsKey.agentHooksAutoInstalled` (JSON `{deviceID: [kind]}`) and installs agents that are available and not yet recorded. An agent is recorded only once the post-install status reports its hooks installed, never merely because it was attempted: installs land per agent, so a batch where Claude Code succeeds and Codex fails must record Claude Code (auto-install is done with it) and leave Codex to retry. Recording the whole batch on success — or none of it when any agent failed — would either permanently give up on an agent that only needed a retry, or reinstall a hook after the user deliberately removed it. The failure messages are stored per (device, agent) in `ClientSettingsKey.agentHooksInstallFailures` and cleared when that agent next installs, so Settings can explain the row. The device round trips run outside the marker lock, which guards only the read-modify-write, so a slow or unreachable remote cannot stall another device's auto-install behind an install timeout; each save merges the device being recorded into the latest setting so overlapping triggers for different devices cannot drop each other's markers. `AppKitController.autoInstallAgentHooks(for:)` guards overlapping work with an in-flight per-device set and probes only the device it was given: a trigger for one device says nothing about whether the others are reachable, and probing them anyway would spend a request timeout apiece on every offline remote, on every edge. Each device triggers it on the edge into a usable daemon (`AppKitController.daemonBecameUsable`): the local device on its first healthy sidebar load, a restart resolving a compatibility block, or a daemon that came back; a remote on pairing success and on overview reconnect. There is no launch-time trigger, because a launch with the local daemon offline or wire-incompatible would consume the device's only pass on a request that cannot succeed.
- Settings → Coding Agents (`SettingsController+CodingAgents.swift`) renders a device picker plus one row per agent (CLI detection + hook status + Install/Reinstall for detected agents), fetching status and installing through the same Device API client so it manages local and remote devices identically. The row stack is seeded before the status request and reused for loading/error states, so transient request failures do not hide the supported-agent list. A row whose agent has a recorded install failure and no installed hooks shows that message in place of its status caption. A manual install answers `ok` even when its one agent failed, so the outcome's per-agent failures decide whether the user sees an error, and the manual path updates the same stored failure map as auto-install. Hook status and installation are reachable only from the app; the CLI and MCP expose neither.
