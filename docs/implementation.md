...
- The macOS remote renderer and the iOS renderer share replay coordination policy in `spacesterminalcore` for history seeds, owner bootstrap preservation, and pending output coverage. Platform adapters keep only the AppKit or UIKit Ghostty surface details local.
...
- iOS render-dump text is derived from the local Ghostty surface after owner bootstrap, incremental output, and scrollback changes. Full output-history is stored as a lazy scrollback seed for the local Ghostty surface, so E2E checks use rendered text, readiness, event logs, screenshots, and performance events for live-output and scrollback coverage.
