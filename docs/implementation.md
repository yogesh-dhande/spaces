...
- Service port assignment is computed in one place and injected into the daemon's process-launch environment. The local macOS daemon also uses those assignments for local Caddy routes. Remote browser forwarding uses the same assigned remote port as the SSH destination and registers the Mac-local forward port as the Caddy upstream.
