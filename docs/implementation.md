...
- The setup step's gate reads This Mac only, never a paired remote: a remote may be asleep, and blocking launch on its round trip would hang the app. `shouldProbeLocalAgents` short-circuits the daemon call entirely once the user has dismissed the step for this hook version, so the steady-state launch costs nothing. A daemon that never answers yields `nil`, which omits the step **without** recording a dismissal — recording one would let a daemon that happened to be down at first launch suppress the step forever. Skip and Continue both record the dismissal, because both mean the user has seen this hook version and decided.
