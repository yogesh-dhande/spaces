...
- The main window and panel windows share one shell recipe: `.titled` without `.fullSizeContentView`, hidden title, transparent titlebar. Content must not extend into the titlebar region — the titlebar's drag machinery intercepts left-clicks there before they reach content views (`mouseDownCanMoveWindow` does not apply to that region), which silently kills interactive chrome like the tab strip. Reclaiming that strip for content requires the Ghostty/Muxy playbook (hide the titlebar decoration views, reposition traffic lights, disable or reimplement window dragging), deliberately not taken on. Overview ticks that land on the already-visible workspace panel refresh only the footer instead of re-embedding the panel view, so transient chrome anchored inside it (the tab rename popover) survives.
