...
- Worktree discovery is event-driven. A FileSystemWatcher (FSEvents) per local git project observes the repository's git common directory and triggers the worktree scan/create/archive reconciliation only when `HEAD` or anything under `worktrees/` changes, so ordinary object and index churn from commits does not wake it. It imports valid unmanaged worktrees for known projects through the same orchestrator reconcile path the watcher targets.
...
- Watchers are owned by the coordinator that owns the affected UI (the worktree, database, and process observers by `AppKitController`/`SidebarController`) with explicit start/stop lifetimes tied to background-service start/stop. When a watcher cannot be installed, the affected live feature surfaces the failure instead of falling back to polling.
