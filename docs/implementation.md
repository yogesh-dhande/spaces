...
The macOS Automations UI is a thin client over the device overview. Each device section's overview carries `automations` and `automationRuns`, so the Automations sidebar row and detail pane read straight from the loaded sections rather than issuing their own list calls. `AutomationsViewModel` (`spacesui`, pure and unit-tested) merges those per-device slices into the automations table, the runs table (newest first by start-or-create time), the running-run set the sidebar row expands, the device filter, the unreachable-device markers, and the failed/timed-out alert entries. Because the pane is overview-derived, it refreshes on the app's existing overview cadence: the host re-invokes `showAutomationsDetail()` and `updateAutomationsSidebarRow()` from the same paths that refresh Alerts.
