...
| Runtime records | `workspace_service_ports`, `runtime_targets`, `browser_targets`, `running_processes`, `agent_sessions`, `agent_session_events` | Live state, kept separate so template edits coexist with running processes |
...
The three terminal commands (`terminal list`, `terminal tail`, `terminal send`) and the agent orchestration commands `agent spawn`, `agent list`, `agent status`, `agent annotate`, `agent interrupt`, and `agent kill` accept a device selector, so they can take the Device API route. `agent subscribe`/`unsubscribe` and `agent signal`, plus project and workspace commands, always target the same-machine daemon over the profile socket. `spaces device list` reads the client database and contacts no daemon at all.
...
Readiness is defined as the first hook-sourced lifecycle event (`lastAgentSignalAt != nil`), not "an agent row exists": foreground detection creates rows without signals, so a row alone is not readiness. The readiness wait is a CLI-side poll (`awaitAgentReadiness`, every 500ms up to the `--timeout` budget), not a blocking daemon RPC, because the daemon handles profile commands serially — a blocking spawn RPC would deadlock against the very `agent signal` it waits for. The CLI and the `spaces_agent_spawn` MCP tool share one `performAgentSpawn` implementation so both block identically. Because agent rows only appear on the first signal, auto-subscribe runs after readiness (the subscription targets the resolved agent-session row id) and `--prompt` injection runs after that, over `.terminalSend`. `agent kill` resolves the Spaces agent row by terminal tracking id across workspaces (`resolveSpacesAgentSession`) and stops it through the coding-agent stop path, which terminates the terminal and deletes the row; before the first signal there is no row, so it terminates the terminal session directly, and a session that is neither an agent row nor a live terminal is a loud error. `interrupt` sends ESC (byte 27) over `.terminalSend`; killing a TUI is `kill`'s job. `subscribe`/`unsubscribe` are persist-only in this stage — they validate the target agent row exists and record/remove the edge through `insertAgentSubscription`/`deleteAgentSubscription`; the injection engine and cycle (acyclicity) validation that consume these edges arrive later.

### Remote orchestration routing (`--device`)

With a device selector, the orchestration commands route through the Device API instead of the profile socket. `spawnAgentSession`, `listAgentSessions`, and `annotateAgentSession` are dedicated Device API commands (`listAgentSessions` is marked replay-safe; the two mutations are not); interrupt reuses `sendTerminalInput` (ESC byte), and remote kill reuses `stopCodingAgent`. The server handlers run the same shared `WorkspaceOrchestrator` logic as the local path — including the identical `AgentSpawnCommandGate` hook gate — so the remote surface enforces the same contracts and reports the same rows (mapped to `SpacesDeviceAgentSessionRow`). Remote spawn differs in one contract: `workspaceID` is required, because a remote client shares no working directory the daemon could infer the workspace from. Remote readiness is still a client-side poll, now against `SpacesDeviceClient.listAgentSessions`, sharing `awaitAgentReadiness` with the local path through an abstracted row-fetch closure.

Two remote limits are deliberate in this stage. First, subscribe/unsubscribe are same-device only: the injection engine that consumes subscription edges runs on the daemon that owns the subscriber terminal, so a cross-device edge would have no engine to deliver it; a remote spawn therefore skips auto-subscribe and prints how to watch the child (poll `agent list --device`, or subscribe from a terminal on the child's device), and an explicit `--device` on `subscribe`/`unsubscribe` is rejected loudly. A client-side relay that mirrors remote lifecycle transitions into a local subscriber is a possible follow-up. Second, remote kill has no path for a pre-signal session: the Device API has no ad-hoc terminal-terminate command, so `agent kill --device` resolves the child's agent row via `listAgentSessions` and stops it through `stopCodingAgent` — a session that has not yet reported its first hook signal has no agent row and cannot be killed remotely, so it fails loudly (interrupt it, or stop it on the child's device) rather than silently no-op.
