...
- The daemon exposes two Device API commands (`SpacesDeviceAPIProtocol.swift`): `agentHooksStatus` (read-only) and `installAgentHooks` (mutation), handled in `SpacesDeviceAPIServer` against the daemon's own home; `SpacesDeviceClient.agentHooksStatus`/`installAgentHooks` wrap them so the same call installs local hooks (the `local` device) or remote hooks (a paired device). The install path rejects requested agents whose executable does not resolve on the daemon host, so config directories created by prior hook installs are never treated as agent detection.
- Auto-install orchestration lives client-side because the app is the single decision-maker. `AgentHooksAutoInstaller` (`spacesclientcore`) reads the per-device recorded set from `ClientSettingsKey.agentHooksAutoInstalled` (JSON `{deviceID: [kind]}`), installs agents that are available and not yet recorded, and records them — so auto-install is once per (device, agent) and never re-adds a user-removed hook, while a newly-available agent is picked up on the next connect. The device round trips run outside the marker lock, which guards only the read-modify-write, so a slow or unreachable remote cannot stall another device's auto-install behind an install timeout; each marker save merges the device being recorded into the latest setting so overlapping triggers for different devices cannot drop each other's markers. `AppKitController.autoInstallAgentHooksForKnownDevices()` guards overlapping work with an in-flight per-device set. Each device triggers it on the edge into a usable daemon (`AppKitController.daemonBecameUsable`): the local device on its first healthy sidebar load, a restart resolving a compatibility block, or a daemon that came back; a remote on pairing success and on overview reconnect. There is no launch-time trigger, because a launch with the local daemon offline or wire-incompatible would consume the device's only pass on a request that cannot succeed.
- Settings → Coding Agents (`SettingsController+CodingAgents.swift`) renders a device picker plus one row per agent (CLI detection + hook status + Install/Reinstall for detected agents), fetching status and installing through the same Device API client so it manages local and remote devices identically. The row stack is seeded before the status request and reused for loading/error states, so transient request failures do not hide the supported-agent list. `spaces agent hooks status` reports local CLI detection and hook status from the CLI process's home; installation is owned by automatic connect-time orchestration and the Settings UI.
