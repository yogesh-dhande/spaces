...
- The iOS renderer reports viewport geometry back to the Mac bridge for owner resizing. The reported rows and render-frame crop come from Ghostty's rendered surface grid sized to the visible area above the software keyboard and accessory toolbar. On iPhone it caps the reported columns to the readable inset surface width so the remote session does not keep emitting columns that render beyond the visible phone edge; iPad keeps the raw column count.
