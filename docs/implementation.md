...
Its stopped-state tint is the `statusFailed` theme token, which the sidebar's `sidebarFailedIndicatorColor()` also reads, so a dead pane and its exited sidebar row cannot drift apart. That token exists because the sidebar's tint was an ad-hoc literal in `SidebarController` and is a rose distinct from the `red` token — matching it by mirroring the numbers into a second module would have drifted on the next retune.
