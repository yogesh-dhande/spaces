...
- Terminal control keeps its flat JSON request shape for wire compatibility while `TerminalControlCommand` provides the typed view used by daemon, Device API, and session-core dispatch. Owner-client gating and post-takeover state inclusion are properties of that typed command rather than repeated raw-string checks.
...
- Window cycling rebuilds the same ordered focusable targets client-side and tracks an in-memory cursor and short-lived cycle session per workspace (no database persistence). The current target is resolved from the focused built-in terminal session, then the frontmost Chrome tab URL when Chrome is focused, then the remembered cursor; the cursor and a per-burst rotation order keep rapid presses stable. Each cycle resolution queries the scoped Chrome snapshot for tracked browser sessions so manual browser navigation outside the app is reflected on the next cycle.
...
- When a device is incompatible, the client is **fully blocked for that one device** (not read-only): no mutations are issued and none of its overview data is rendered (an overview read may be attempted, but an undecodable or incompatible result is discarded in favor of the block), its detail surface shows the compatibility block with the restart-impact report, and other paired devices stay fully functional. A local TerminalService compatibility failure throws `TerminalServiceError.daemonWireIncompatible` with the verdict, daemon status, and message so the macOS app can mark the local device incompatible and open the same compatibility block instead of showing a generic error alert. When compatible, the client never forces a restart; if the daemon reports an older app version than the client, a quiet "update pending" indicator notes that the staged update applies on the next natural restart.
- During macOS startup, a local `TerminalServiceError.daemonWireIncompatible` from the initial sidebar bootstrap is handled as the detail-pane compatibility block and returns before the generic startup placeholder path for both `daemonTooOld` and `clientTooOld` verdicts. The block hydrates the local paired-device record from the client database before rendering so the `daemonTooOld` Restart action can route through the normal `requestDaemonRestart` Device API path even though the snapshot did not populate `localPairedDevice`; `clientTooOld` renders the same block without a restart action.
- Restart authority by device: every daemon restarts through the `requestDaemonRestart` RPC, which terminates the daemon gracefully so launchd `KeepAlive` / systemd `Restart=always` respawns it. A local or remote Mac applies any update staged in `Spaces.app` on that respawn because launchd starts `~/.spaces/bin/spacesd`, and that helper points to `Spaces.app/Contents/Resources/spacesd`. A Linux daemon respawns the same binary, so updating a Linux daemon is not a restart action: the wire-incompatible compatibility block for a Linux daemon shows the version-pinned `spaces-install-linux.sh` one-liner for the user to run on the Linux device, and Spaces never updates it over SSH. The macOS app launch path repairs Spaces-owned helper symlinks and the LaunchAgent plist when the app runs from `/Applications/Spaces.app`, but it does not unload, kickstart, or restart `spacesd`; the restart remains explicit because it terminates daemon-owned terminals and runtime processes. iOS cannot refresh a daemon binary (no SSH): for a remote Linux daemon (identified by the status `operatingSystem`) it directs the user to update from the Mac app instead of offering a restart that would respawn the same build; for other devices it can request a restart over the RPC to apply an already-installed update. A daemon restart is destructive — it terminates the daemon's terminal, process, and coding-agent children — so every restart path first surfaces the impact counts and lets the user defer.
...
- `WorkspaceRuntimeTargetIndex` materializes the shared ordering used by sidebar runtime rows, command-palette items, numbered shortcuts, and window cycling, and exposes lookup maps by process ID, terminal session ID, agent ID, URL, and shortcut identity so those surfaces do not rescan the same overview rows differently.
...
- Window cycling is built from the same workspace target model as numbered shortcuts, then filtered to targets with an already-open client window. Browser targets come from `ClientBrowserWindowIDStore` entries whose tracked Chrome window still contains a tab matching the resolved session URL, queried with a single scoped Chrome snapshot that also carries the frontmost active-tab URL for current-target resolution; unopened browser-session configuration rows, unadopted matching tabs in untracked Chrome windows, and stale tracked IDs are skipped. Terminal-backed process, ad hoc terminal, and coding-agent targets are included only when `PanelCoordinator.openTerminalSessionIDs(workspaceID:)` reports an open pane for that session. Not-yet-running process rows and configured agent launchers remain direct-focus launch actions and do not participate in cycling.
...
- `spaces agent signal` resolves workspace and terminal-session IDs from explicit `--workspace`/`--session` arguments or, when omitted, from `SPACES_WORKSPACE_ID` and `SPACES_TERMINAL_TRACKING_ID`. When no argument is given and the environment supplies neither ID, the command exits successfully without contacting the daemon, which keeps globally-installed agent hooks harmless outside Spaces-managed terminals. Supplying one ID explicitly and not the other throws a validation error instead: the caller named a session, so a silent no-op would hide their mistake. Hook installation and status are not exposed through the CLI or MCP; the app and the daemon own them. `init` creates or attaches the originating terminal's agent row. Non-`init` events update an existing row, or establish one when configured session metadata or current terminal runtime identifies the terminal as a coding agent. The label inference path recognizes configured Spaces agent terminals and known runtime agent foreground state.
...
- `AgentHookInstaller` (`spacesterminalcore/AgentHooks/`) installs and reports the lifecycle hooks each supported agent needs to call `spaces agent signal`. It lives in `spacesterminalcore` because it cross-compiles to Linux and is invoked by the daemon on both the local Mac and remote hosts; the installer only ever writes into the home directory of the machine it runs on. `SupportedCodingAgentHook` (`claudeCode`/`codex`/`opencode`) owns each agent's config paths, executable-only availability probe, and event bindings. Availability first checks the daemon PATH and common executable directories, then asks the daemon user's interactive login shell for its PATH so shell-managed installs from version managers are detected before the agent is marked unavailable. That shell probe costs seconds (it sources the user's whole rc chain), so one resolver serves an install and the status it returns, and the answer is cached per home for a short TTL on a monotonic clock. The TTL is what keeps "a newly installed agent is picked up on the next connect" honest: `spacesd` outlives the app, so a cache held for the daemon's lifetime would survive an app relaunch and hide an agent installed into a version-manager shim directory that is not yet on the daemon's PATH. Failed probes are cached too, or a missing shell (or an rc script that hangs past the probe timeout) would pay that timeout on every status call. The probe drains the shell's output while it runs (an rc chain writing past the 64KB pipe buffer would otherwise deadlock) and reads through to EOF before parsing, because the PATH marker line is printed last. Its stdin is `/dev/null`: the shell is interactive so version-manager `.zshrc` lines apply, and an interactive shell inheriting a terminal would reconfigure that terminal's modes or take job-control signals that stall the probe until it times out.
- Claude Code and Codex share one JSON hook file shape (`hooks.<Event>` arrays of `{matcher, hooks:[{type:"command", command}]}`), so `AgentHookJSONWriter` handles both `~/.claude/settings.json` and `~/.codex/hooks.json`; Codex additionally needs `features.hooks = true`, applied by `AgentHookCodexFeatureToggle` as a table-aware line edit for `[features]` sections, top-level `features = { ... }` inline tables, and top-level `features.*` dotted keys (no full TOML rewrite). Table headers and keys are matched by name, not literal text, so `[features]`, `[ features ]`, and `["features"]` are all recognized as the same table: an unrecognized header would fall through to appending a second `[features]` table, and the duplicate definition would make Codex fail to parse `config.toml` at all. For the same reason the toggle splits lines on either newline TOML allows, `\n` or `\r\n`, and rejoins with the one the config already used: a CRLF config read as LF leaves a `\r` on every line, which hides `[features]` from the header match and makes `true` parse as neither `true` nor `false`, so Spaces would append that duplicate table to a config Codex reads perfectly well. When `features` is already defined in a shape the line editor cannot extend — `[[features]]`, or a top-level `features` key that is not an inline table — the toggle throws rather than append a colliding table, so the install fails loudly instead of costing the user their Codex config. `features.hooks` already spelled as a *table* is rejected separately and earlier, by `conflictingHooksTableDefinition`, before any edit path runs: `[features.hooks]`, a deeper `[features.hooks.matchers]`, `[[features.hooks]]`, `hooks.enabled` inside `[features]`, and top-level `features.hooks.enabled` all make `hooks` a table, and every edit path writes that one key as a boolean. Checking once up front is what makes that safe — the append path's own conflict scan never sees these, because a `[features]` table or a dotted `features.*` anchor diverts the edit before it. A `hooks` key holding anything but `true`/`false` throws for the same reason: `hooks` is a boolean feature flag, so any other value means a config shape the editor does not understand, and rewriting it to `true` would silently destroy user data. The Codex config edit treats only a missing config file as empty; read, permission, encoding, or other I/O failures throw and leave the file untouched. opencode has no shared hook file, so `AgentHookOpencodePluginWriter` writes a Spaces-owned plugin under `~/.config/opencode/plugin/` (opencode auto-loads that directory); the plugin reports via Bun's `$` shell helper. Generated hooks use `'<path>/spaces' agent signal <event> >/dev/null 2>&1 || true # spaces-agent-hook`, discard stdout/stderr, and end with `|| true` so transient Spaces failures do not fail the agent hook. The Spaces CLI is resolved through the same `ExecutableResolver` at install time and written in as a shell-quoted absolute path, because an agent runs hooks with whatever environment it inherited: a bare `spaces` that did not resolve would, given the discarded output and the `|| true`, leave every hook a silent no-op that still reported as installed. An install whose host has no `spaces` throws `AgentHookInstallerError.spacesCLINotFound` and writes nothing; moving the CLI requires reinstalling hooks. The opencode plugin embeds the same path as a JS constant and passes it through Bun's `$` helper, which quotes it as one argument.
- Every writer writes through `AgentHookConfigFile`, which follows the destination's symlink chain before writing. Agent configs are commonly symlinked into a dotfiles repo, and an atomic write (temp file renamed over the destination) would replace the symlink with a regular file and silently detach the managed config. A link is followed only when its destination exists: a dangling link (repo not cloned, unmounted volume, or a cycle, which `stat` reports as nonexistent) points at nothing worth preserving, so the write replaces the dead link rather than creating directories at a destination the user never populated. Regular files and missing files resolve to themselves.
- Idempotency is by construction: each writer ensures desired state rather than appending. The JSON writer strips Spaces-owned entries (identified by the `# spaces-agent-hook` command marker) from every event, removing only those entries inside mixed hook groups before re-adding one Spaces entry per mapped event, so unrelated hooks and keys are preserved; the opencode plugin is a whole-file overwrite; the Codex toggle only writes when the flag is not already set. Replaying an install yields byte-identical output, and a reinstall replaces a Spaces entry carrying a stale CLI path. A malformed existing JSON config is left untouched (that agent's install fails rather than clobber it).
- Hooks carry the version that wrote them: `AgentHookCommand.hookVersion` (SPACES_HOOK_VERSION), rendered into every generated command and the opencode plugin header as `# spaces-agent-hook v<N>`. Bump it by hand whenever the generated command, the plugin body, or an agent's event bindings change; a config carrying an older number then reads back as `.outdated` and the UI offers Update. This is what lets a Spaces release correct hooks a previous release installed — without it, hooks installed once could never be fixed.
- Ownership and currency are deliberately different predicates. `AgentHookCommand.isSpacesOwned` matches the version-less `# spaces-agent-hook` token and is the *only* thing the writers' stripping path consults, so a reinstall removes entries written by any version. `isCurrent` parses the version and is used only on the status path. If stripping ever became version-aware, an old entry would survive the reinstall and a second entry would be appended beside it, leaving one stale hook firing per Spaces upgrade. `embeddedVersion` reads the full digit run so `v1` is not matched as a prefix of `v10`.
- `AgentHookInstallState` is `.notInstalled` when no Spaces-owned entry exists for the agent, `.current` when every bound event carries a current-version entry, and `.outdated` for everything between — an older version marker, or a bound event this build added that the installed config never covered. Codex composes two files: current `hooks.json` entries with `features.hooks` off are `.outdated`, since the hooks exist but cannot fire and reinstalling sets the flag. (A Codex install whose `config.toml` edit fails after `hooks.json` was already written lands in exactly this state.)
- Event → signal mappings: Claude Code `SessionStart/UserPromptSubmit/PermissionRequest/Stop/SessionEnd` → `init/working/blocked/done/exit`; Codex the same minus `exit` (no session-end event); opencode plugin startup → `init`, `chat.message` → `working`, event `permission.asked` → `blocked`, event `session.idle` → `done`.
- The daemon exposes two Device API commands (`SpacesDeviceAPIProtocol.swift`): `agentHooksStatus` (read-only, answering `SpacesAgentHooksStatusPayload`) and `installAgentHooks` (mutation, answering `AgentHookInstallOutcome`), handled in `SpacesDeviceAPIServer` against the daemon's own home; `SpacesDeviceClient.agentHooksStatus`/`installAgentHooks` wrap them so the same call installs local hooks (the `local` device) or remote hooks (a paired device). Agents install independently: an agent whose executable does not resolve on the daemon host, or whose config cannot be edited, becomes an `AgentHookInstallFailure` in the outcome rather than a rejected request, so its siblings still install and the client learns which agents landed. (Requiring the executable to resolve is what keeps config directories created by prior hook installs from being treated as agent detection.) Only a missing Spaces CLI rejects the whole request, mapped to `capabilityMissing` because the request was well formed and the host lacks what every hook command needs.
- Installs are always user-initiated. `CodingAgentsView` (`spacesui`) owns the device picker, the status fetch, the per-agent Install/Update/Reinstall action, and the rows; Settings → Coding Agents and the launch setup step both embed it, so local and remote hooks are managed by one code path over the same Device API client. It depends on the narrow `CodingAgentsHost` protocol rather than `AppKitController`, because the setup screen runs before the main window content exists. The row stack is seeded before the status request and reused for loading/error states, so a transient request failure does not hide the supported-agent list. A manual install answers `ok` even when its one agent failed, so the outcome's per-agent failures decide whether the user sees an error. Install failures are held in memory, not persisted: every install is user-initiated, so the message is on screen when it happens and cannot outlive the problem it describes. Hook status and installation are reachable only from the app; the CLI and MCP expose neither.
- `SetupFlowController` (`spacesui`) sequences the launch steps and replaces the window's content until they are done, then hands back to `AppKitController.presentMainWorkspaceUI`. The Chrome Automation step blocks; the coding-agents step does not. `requiresCodingAgentsSetup` is a pure decision over the local device's `[AgentHookStatus]`, the dismissed hook version (`ClientSettingsKey.agentHooksSetupDismissedVersion`), and the current hook version.
- The setup step's gate reads This Mac only, never a paired remote: a remote may be asleep, and blocking launch on its round trip would hang the app. `shouldProbeLocalAgents` short-circuits the daemon call once the user has dismissed the step for this hook version, which is the only answer a probe could not overturn. Every other launch pays one `agentHooksStatus` request, because whether the user has since installed a coding agent is exactly what the client cannot know locally; the request overlaps the daemon bootstrap launch performs anyway for the sidebar. A daemon that never answers yields `nil`, which omits the step.
- The dismissal marker (`ClientSettingsKey.agentHooksSetupDismissedVersion`) is written from exactly one place, the step's Skip/Continue action, and means "the user saw this hook version and decided". It must never be written for "Spaces found nothing to install": a launch that records a dismissal because no agent was detected retires the step permanently on that machine, so a user who installs Claude or Codex the next day is never offered it — only a hook-version bump would reset it. The same reasoning covers an unreachable daemon, which is why `nil` records nothing either. The cost of that correctness is a status request per undismissed launch; the alternative is a marker that lies.
- The probe resolves the local device through `SpacesDeviceClient.bootstrapLocalDevice` rather than reading the stored paired-device record, because it is the launch's first daemon call and only bootstrapping starts `spacesd`. A plain Device API request re-bootstraps only to recover a *missing* local auth token (`credentialsEnsuringLocalRecovery`), so once a token is on disk the request dials the record's endpoint and spends its entire timeout against a daemon nobody started, reporting no agents. Bootstrapping starts the daemon and returns its current host, port, and certificate fingerprint in one step — which matters whenever the stored record predates the last daemon restart, most visibly under a dev profile, where `scripts/dev-build-and-launch.sh` sets `SPACES_DEVICE_API_PORT=0` and the daemon binds a fresh ephemeral port each start (installed builds keep the fixed `SpacesDeviceAPIEndpointDefaults.port`).
- The probe starts in `SetupFlowController.begin()`, before the first step renders, so a cold `spacesd` warms up while the Chrome Automation screen is on-screen instead of after it. Waiting for it is bounded by `localAgentStatusTimeout`, which must stay larger than the Device API's own request timeout — a smaller budget abandons the probe before the request it waits on can succeed or fail, silently dropping the step on exactly the cold-daemon launch that most needs it. The Device API call is blocking and cannot be cancelled, so the timeout stops *waiting* on it rather than stopping it; the orphaned read-only request is harmless.
