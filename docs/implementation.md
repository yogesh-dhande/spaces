...
- `spacesterminalghostty`: embedded libghostty integration for service-owned and legacy in-process sessions, including local artifact discovery, runtime callbacks, control-plane bridging, and lightweight app-side host adapters for client windows.
...
- `GhosttyEmbeddedSessionHost` in `spacesterminalghostty` is the service-owned session runtime for `ghostty-embedded`. It owns the PTY, libghostty state, output logging, runtime-state refresh, active owner identity, host-stamped lease timestamps, and lease expiry for stale remote clients.
...
- Owner bootstrap uses a Ghostty session snapshot for both macOS windows and mobile clients. Remote mobile owner bootstrap uses the cached handoff snapshot captured before transfer, while macOS local-window owner bootstrap exports the live service session snapshot on initial attach and ownership changes. The VT snapshot stream remains a fallback, not the primary owner first-paint path.
- Owner resync, resize, and live-output rendering do not call `ghostty_session_export_snapshot`; live output after bootstrap is streamed as incremental bytes. This keeps the high-fidelity snapshot export bounded to owner bootstrap and final render states while avoiding stale replay after Mac/iOS ownership changes.
- `TerminalSessionWindowController` uses `RemoteGhosttySessionHost` for service-owned sessions by default. When a legacy in-process host already exists for that session ID, it can still reattach the local Ghostty surface; otherwise it follows the service-owned control plane, consumes the daemon snapshot stream for owner handoff compatibility, metadata, and ended-session final renders, and sends terminal input through the shared serial input queue. Live non-owner windows stay on takeover or status UI instead of mounting a passive terminal surface.
