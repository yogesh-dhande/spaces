...
- Auto-install orchestration lives client-side because the app is the single decision-maker. `AgentHooksAutoInstaller` (`spacesclientcore`) reads the per-device recorded set from `ClientSettingsKey.agentHooksAutoInstalled` (JSON `{deviceID: [kind]}`), installs agents that are available and not yet recorded, and records them — so auto-install is once per (device, agent) and never re-adds a user-removed hook, while a newly-available agent is picked up on the next connect. Auto-install runs serialize their load/install/save sequence, and each marker save merges the device being recorded into the latest setting so overlapping triggers for different devices cannot drop each other's markers. `AppKitController.autoInstallAgentHooksForKnownDevices()` guards overlapping work with an in-flight per-device set and drives it from three triggers: local bootstrap at background-services start, remote pairing success, and remote overview reconnect.
