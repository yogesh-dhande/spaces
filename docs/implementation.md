...
The session picker behind split-right/split-down, the tab strip's `+`, and the new-tab shortcut (default `⌘T`) excludes sessions already occupying a pane anywhere, sourced from `PanelCoordinator.openSessionIDs()` over in-memory layouts (no DB read; the selected workspace's layout is always restored before its panel shows, and global panel windows restore eagerly at startup, so a `.workspace`-scope picker is exact — a `.globalWindow` picker's only approximation is a session persisted in a never-selected workspace's unmaterialized layout, the same one `fillSplit`'s move branch already lives with). `AppKitController.sessionPickerPresentation` splits into a `nonisolated static` core that builds the rows from already-gathered overviews and the occupancy set (so it's testable without a live controller) plus a thin instance wrapper that gathers them. `⌘T` and the `+` button both call `presentNewTabSessionPicker`, landing the chosen or freshly created session as a new tab through `PanelCoordinator.openSessionInNewTab`; the `New terminal` leader shortcut keeps calling `openNewTerminalTab` directly, bypassing the picker. `fillSplit`'s remove-then-insert branch stays only as a race guard — a session opening elsewhere while the picker is up — since the picker itself no longer offers sessions that are already open.
