...
- `spaces signal` only resolves direct built-in terminal environments. The label inference path recognizes Codex, Claude Code, and opencode-specific environments, while `SPACES_AGENT_LABEL` remains the explicit label override for custom agent hook integrations.
