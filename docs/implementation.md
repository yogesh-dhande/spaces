...
`AutomationService` (`workspacecore`) is the daemon-side scheduler and executor: one instance drives cron firing, missed-run catch-up, the concurrency gate, execution, timeout/cancel teardown, and retention, all through a poll-based `tick()` so no path depends on background tasks. Two edges of that design are intentional rather than bugs to chase: a `running` run row left behind by a daemon crash is not reconciled at startup — `reconcileMissedRunsOnStart()` runs once before the tick timer starts, so a cron automation whose concurrency policy is `skip` or `queue` can see that stale row as still active and defer its catch-up run until the first `tick()`'s `pollRunningRuns()` finds the row's session gone and finalizes it; and retention (`pruneRetention`, newest 100 runs kept) counts only terminal runs, so a queued or running run does not count against the cap and the automation's total row count can momentarily exceed 100 while one is in flight.

An automation's `kind` decides what a run's terminal session is. A `script`-kind run executes in a workspace-less command session — excluded from every workspace's `sessions` for the same reason any other workspace-less session is (see the overview note below) — wrapped in a POSIX-sh script (`wrappedCommand`) that exports `PATH` so `spaces` resolves, runs the user's script in a `( … )` subshell (so a script that calls `exit` only ends the subshell), and writes the exit code to a per-run sentinel file the poller reads to detect completion without depending on a signal. An `agent`-kind run instead spawns straight into the chosen workspace through `createWorkspaceAgentSession` → `launchWorkspaceCommandSession` — the same path `spaces agent spawn` and any hand-launched terminal command use, including the workspace `WindowRecord` insertion that marks the workspace running — so the run's terminal session IS the agent's real session in that workspace: it shows up in the workspace's own panels and window cycling, and in Coding Agents and Alerts, indistinguishably from an agent a person started themselves. Only the `automationRunID` stamped on the session, and the attributed-agents mapper below, tie it back to the run for the Automations pane. Attribution is daemon-local: a run id names a row in the owning daemon's database only, so an automation script that spawns an agent on another device (`spaces agent spawn --device`) does not forward the run id, and that remote session is not attributed to the run.

`launchAgentRun` only spawns the agent and records its session id; it never sends the prompt itself, so `tick()` never blocks on foreground detection. `pollRunningAgentRun` drives the rest through two phases keyed off the persisted `AutomationRun.promptDeliveredAt`, so a daemon restart resumes the correct phase from the store instead of a parallel in-memory recovery path: `nil` is the detecting/sending phase (`pollAgentDetectionPhase` polls the session's `foregroundDetectedAgentKind` up to a 90s deadline — the same default `spaces agent spawn` uses — and once detected, `deliverAgentPrompt` writes the prompt text, then a separate CR byte, as two independent writes; a single write with a trailing CR is exactly what OpenCode leaves unsubmitted, so the CR is always its own write, and `promptDeliveredAt` is persisted only after it succeeds, so a restart mid-delivery retries the whole send rather than resuming into a half-submitted prompt); once set, `pollAgentAwaitingPhase` awaits the first of the agent's orchestration row reaching `done` (→ `succeeded`, the session deliberately left open — see below) or the session ending on its own (→ `succeeded` unless the terminal backend recorded a `.failed` end state, since a deliberate close is the common case and the backend has no numeric exit code for a normal agent exit). `deliverAgentPrompt` writes through `WorkspaceOrchestrator.writeAutomationSessionInput`, a thin seam over the process-wide terminal-session input writer the daemon installs; a non-daemon caller (which never spawns agent automations) has none installed and the write throws instead of silently no-op'ing, and routing through this seam keeps prompt delivery identical to any other programmatic terminal input such as `spaces terminal send text`.

A `done` signal completes the run immediately and leaves the agent's session running: the run's job is only to track whether the seeded work started and finished, while the session is what the person actually wants to keep looking at, so ending the run must never end the session. This decoupling is also why the concurrency gate treats a live attributed session as blocking in its own right, not just the run row's `running` status — `automationHasLiveAttributedSession` walks every prior run of the automation and checks whether any attributed session is still live. A `skip`/`queue` automation therefore keeps recording skipped rows (or holding its queued run) while yesterday's spawned agent session is still open, even though that run itself finished `succeeded` hours ago; the daemon never kills a live agent to make room for a new run, so closing the session by hand, or End-agents, is the only release valve (`allow` is unaffected, since it never checks for blocking).

`AutomationAttributedAgents.summariesByRunID` (`workspacecore`) is the one mapping every transport uses to compute a run's attributed-agent breakdown, so a run's coding agents read identically wherever they are listed: the profile-socket runs listing, the Device API runs listing, and the device overview each call it rather than deriving their own. It batch-fetches every workspace's agent rows once (keyed by `terminalTrackingID`) and, for each run's attributed session ids, surfaces a session as an agent when it either has an orchestration row (the agent has signaled or been foreground-detected) or is a still-live `.agent`-kind session with no row yet (detection/prompt-delivery in flight, reported as a row-less `idle`+`live` entry); a session that is neither — the run's own script-kind session, or an already-ended agent session pending the sweep — is omitted.

`endAttributedAgents` (End agents) only accepts a terminal run; a still-running or queued run must be canceled instead, since cancel and End-agents tear down through different paths and mixing them would leave a run's status ambiguous. For each of the run's still-live attributed sessions it captures the transcript, then goes through the same agent-kill flow cancel/timeout use (finalizing the agent row and notifying subscribers), falling back to a plain session termination only for a not-yet-signaled `.agent` session with no row. The run's status, exit code, and timestamps are left untouched — End agents reaps lingering sessions, it never re-finalizes the run. After ending those sessions it prunes the automation's retention, so a run retained past the cap solely because its agent was live is removed in the same operation rather than waiting for a later run to prune; because that prune can delete the very run being ended, End agents returns the run's last-known (untouched) state rather than re-reading a row that may be gone. Removing a run's records (`deleteRunArtifactsAndSessions`, shared by automation deletion and retention pruning) first terminates any still-live attributed session and finalizes its agent row before deleting the row/directory, so a succeeded agent run's deliberately-open session can never be orphaned into a running-but-unreachable process; it captures no transcript, since the run's whole artifacts directory is removed in the same call. Because that removal terminates live sessions, `pruneRetention` skips any prunable run whose attributed session is still live: pruning it would kill a running coding agent, violating the no-kill guarantee (a succeeded agent-kind run deliberately leaves its agent open). Such a run stays retained past the cap until its session ends — then a prune removes it, whether that end comes on its own (removed by a later run's prune) or from the user running End agents (which ends the session and prunes in the same call); whole-automation deletion still terminates, because it is user-initiated. `updateAutomation` rejects a Script↔Agent kind change while the automation has a queued or running run, because `pollRunningRun` dispatches on the automation's current kind and a mid-run switch would misclassify the in-flight run; every other field stays editable.

A timeout/cancel teardown of a `script`-kind run's command SIGTERMs the command's process group, then escalates to SIGKILL after a grace (`processPendingKills`). The pending kill captures the process group id (`PendingKill.processGroupID`) at SIGTERM time — when the child is its own group leader and that group is not the daemon's own — rather than recomputing it at the deadline. A run stays pending while EITHER the leader pid or (when a group was captured) any process in that group is still alive, and the escalation SIGKILLs the captured group and the leader pid. This is what reaps a descendant that ignored SIGTERM after the shell group leader has already exited: keying escalation on the leader pid alone would drop the pending kill when the leader died and leave the survivor running, and recomputing `getpgid(childPID)` at the deadline would fail (or, after pid reuse, name the wrong group) once the leader is gone.

The full agent-kind lifecycle — spawn, foreground detection, prompt delivery, `done` completion, End-agents — is exercised by `workspacecoreTests/AutomationServiceTests`, not `e2e_automations.sh`: `AgentSpawnCommandGate` accepts a fixture's command string (e.g. `claude`), but the daemon's foreground classifier (`TerminalForegroundAgentReconciler`) matches the live process's resolved executable and `argv[0]` basename, and a shebang fixture script named `claude` runs as its interpreter (`/bin/bash <path>/claude`) — the detected executable is `bash`, never `claude` — so no fixture can be foreground-detected without a real provider on `PATH`. The e2e script instead drives the deterministic parts of the agent path directly: a clean spawn failure (an unresolvable workspace id) records `failed` with no attributed agents, and End-agents on that terminal run is confirmed to be an accepted no-op that leaves the run `failed`.

The device overview's `automationRuns` is bounded but never drops a run a client needs. `SpacesDeviceOverviewBuilder.selectOverviewRuns` (pure, unit-tested) unions three store queries, de-duplicated and newest-first: the newest terminal runs globally (`terminalAutomationRuns`, the recent-history window capped at `recentAutomationRunLimit`), each automation's single newest terminal run (`latestTerminalAutomationRunPerAutomation`), and every currently-active run (`activeAutomationRuns`). The latest-per-automation union is what keeps every automation's last-run status accurate: without it, one chatty automation could fill the global recent window and evict the quieter automations' rows, making them read as "never run" with empty history even though the daemon still retains their runs. Per-automation run-history screens do not rely on the overview window at all: the Device API `listAutomationRuns(automationID:)` read command (replay-safe) returns the automation's full retained run set from `store.automationRuns(automationID:)`, with the same attributed-agent breakdown, so a history screen reads the daemon's retained rows directly rather than whatever happens to be in the overview snapshot.

The macOS Automations UI is a thin client over the device overview. Each device section's overview carries `automations` and `automationRuns`, so the Automations sidebar row and detail pane read straight from the loaded sections rather than issuing their own list calls. (The Mac's Runs tab is a global merged list served by the overview; the per-automation `listAutomationRuns` endpoint has no Mac consumer yet, but exists on `SpacesDeviceClient` alongside iOS's.) `AutomationsViewModel` (`spacesui`, pure and unit-tested) merges those per-device slices into the automations table, the runs table (newest first by start-or-create time), the running-run set the sidebar row expands, the device filter, the unreachable-device markers, and the failed/timed-out alert entries. Because the pane is overview-derived, it refreshes on the app's existing overview cadence: the host re-invokes `showAutomationsDetail()` and `updateAutomationsSidebarRow()` from the same paths that refresh Alerts.

Mutations (create/update/delete/run-now/cancel, and the enable toggle) go through `SpacesDeviceClient`, which treats the local daemon as just another paired-device record, so local and remote automations take one path. Those responses carry no overview, so after a mutation the controller requests a sidebar reload (forcing a remote refresh for a remote device) and lets the overview-driven re-render update the pane, rather than merging an optimistic local copy.

A script-kind run's workspace-less session is deliberately excluded from the overview builder's workspace-scoped `sessions`, so the normal session→pane resolution (which reads `overview.sessions`) cannot find it; an agent-kind run's session, having a real workspace id, is not excluded and resolves normally. Either way, opening a run's terminal from the Automations pane synthesizes a `DeviceTerminalOpenRequest` from the run (or, for an attributed-agent chip, from the agent) and opens it in its own standalone panel window rather than a workspace pane, since the Automations pane has no workspace pane to host it in. The seeded shell is display metadata only — the pane attaches to the daemon's existing session and streams its real render — so a running run shows a live pane and an ended one shows the read-only transcript replay through the same attach path used for any ended pane.

The cron schedule builder is `AutomationSchedulePreset` (pure, unit-tested): each preset serializes to a canonical cron string and re-derives from a string only when it matches that preset's exact shape, otherwise falling to Advanced with the string verbatim. The cron string is the only stored form; the editor validates it locally (and previews the next three fires via `AutomationCronSchedule`) before sending, so a parse error surfaces inline instead of round-tripping. Cron anchors (`nextFireTime`) are absolute instants computed in the device's current time zone; each `tick()` compares the daemon's current zone against the zone the anchors were last computed in and recomputes every enabled cron automation from now when it changed, so a device carried into a new zone keeps firing daily/weekly schedules at the new zone's wall-clock time without a restart.

Because the daemon evaluates cron in ITS device's zone, the editor previews next-run times in that zone rather than the Mac's. The daemon reports its current zone in `TerminalServiceDaemonStatus.timeZoneIdentifier` — read fresh (`NSTimeZone.resetSystemTimeZone()` then `TimeZone.current.identifier`, consistent with the scheduler's zone provider) where the daemon builds its status, and added with `decodeIfPresent`/optional so the frozen-core handshake still decodes an older peer that omits it. The field rides the overview's inline `daemonStatus`; `AutomationDeviceInput.timeZoneIdentifier` threads it to the editor, and `AutomationsViewModel.schedulePreviewTimeZone` resolves the preview zone (the Mac's `.current` for the local device, else the reported zone, falling back to `.current` when unreported/unparseable). `schedulePreviewZoneSuffix` appends the zone identifier to the preview text only when it differs from the Mac's, so local authoring stays uncluttered.

The iOS Automations feature (`apps/ios/Sources/Automations/`) is the same overview-derived read model narrowed to view/trigger/cancel: `SpacesMobileAutomations` (pure, unit-tested) derives the automations list and a run list from `SpacesMobileAppModel.overview.automations`/`.automationRuns` — the same fields the macOS overview carries — with no separate list call. Unlike `AutomationsViewModel`, it takes a single overview rather than an array of per-device slices: the iOS app is a single-active-device client (`SpacesMobileAppModel` holds one `overview`, not one per paired device), so there is no cross-device merge to do and no device column to show; switching the active device discards the previous overview and loads the new device's own automations. The wire summaries (`TerminalServiceAutomationSummary`/`Run Summary`) are designed to need no `workspacecore` dependency (their enum-typed fields are already raw strings), which iOS relies on directly since it does not link `workspacecore` — `SpacesMobileAutomations` and `StatusDot.Kind(automationRunStatus:)` switch on those raw string values instead of the `workspacecore` domain enums the Mac UI uses. Run-now and cancel go through two new `SpacesDeviceAPIClient` methods that reuse the `triggerAutomation`/`cancelAutomationRun` `SpacesDeviceAPICommand` cases already shared with macOS's `SpacesDeviceClient`; those responses carry no overview (only the affected run), so `SpacesMobileAppModel.triggerAutomation`/`cancelAutomationRun` explicitly `refresh()` afterward, mirroring the Mac controller's post-mutation sidebar reload. There is deliberately no toast for a triggered run's started/queued/skipped outcome: the automation row's status dot updates once the refreshed overview lands, the same "let the row reflect it" surfacing the Mac pane uses. The per-automation "View Runs" screen fetches its rows through `SpacesDeviceAPIClient.listAutomationRuns(automationID:)` (a read, not a mutation) when opened and on pull-to-refresh, so it shows the daemon's full retained run history for that automation rather than only the overview's recent-runs window; the global "Recent Runs" screen stays overview-derived, since it genuinely is the recent-runs window. Automations are a headline feature, so `AutomationsTabView` gives the feature its own bottom tab (`RootTabView`, between Agents and Settings) rather than a section of Settings or of the workspace-scoped Alerts/Spaces/Agents tabs; `AutomationsListView` is its reusable root content (unit tests still construct it directly, without a navigation container), and the tab reuses `OverviewPollingModifier` keyed to `.automations` so it polls only while actually on screen. The tab badge is `SpacesMobileAutomations.runningCount`, a pure count of in-flight runs across every automation on the device (SwiftUI's `.badge(_:)` hides it at zero); a failed/timed-out run does not add to this badge, since that outcome already badges Alerts. Failed/timed-out automation-run alerts are derived separately (`SpacesMobileAutomationAlerts`) from the same workspace-grouped attention derivation (`SpacesMobileAttention`), since automation runs are workspace-less and cannot join a workspace group; the Alerts tab renders them as their own trailing band.
...
`terminal send text`'s `--submit` flag (and the `spaces_terminal_send` MCP tool's `submit` argument) issues the text send and then a second, wholly independent send of a lone CR byte (0x0D), both through the same local-or-device `sendTerminalInput` routing as any other send. This is a distinct mechanism from `--newline`/`appendNewline`'s single-call, server-side spaced text+CR write (see the notification-delivery send chokepoint elsewhere in this document): OpenCode's composer does not treat that single call's internally spaced CR as a submit, but does treat a genuinely separate follow-up send as one (issue #187). `--submit` and `--newline` compose: `--newline` still governs whether the text send itself appends a newline, and `--submit` always adds the trailing independent CR send regardless.
