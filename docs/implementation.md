...
- The daemon advertises `protocolVersion`, its host `operatingSystem` (`macOS`/`Linux`), and `activeSessionCount` on `TerminalServiceDaemonStatus`. `SpacesWireCompatibility.evaluate` compares the daemon's `protocolVersion` against the local `SpacesWireProtocol.version` and returns `compatible`, `daemonTooOld`, or `clientTooOld`.
...
- Before touching any live session, the old image runs the staged binary once as a child with `--handoff-check <formatVersion>` (`DaemonHandoffPreflight`) and proceeds only on exit 0 — confirming the staged binary can read the table format about to be written, without an ongoing negotiation protocol. `requestDaemonRestart()` also refuses the handoff when `DaemonHandoffDecision.refusesExecByGenerationGuard` says so: the table carries a `generation` counter that advances by one on every handoff; once it reaches 3, a further handoff is refused only while the version that handed off to the running image equals the running version (an exec-loop guard between two bad builds), so a genuine version change — including the first handoff of a run, where there is no prior source version — always proceeds regardless of the count. Every failure path leaves the daemon fully functional: a refused generation guard or a failed preflight returns before anything is stopped, and a failed table write or a returned `execv` call rebinds every quiesced session in place (`resumeInPlaceAfterFailedHandoff`) and restarts the shared services that were stopped — nothing was freed, so this is a rebind, not a rebuild. A resume that crashes in the new image instead falls to the ordinary supervisor respawn and `recoverStaleSessions()`, the same orphan-session recovery that predates this feature (sessions land `.failed`).
...
- `.applyStagedUpdate` is a frozen `TerminalServiceCommand` case on the local (same-machine) profile socket, distinct from the Device API's frozen core: it is the one poke that asks a daemon to run its own `requestDaemonRestart()`, answered synchronously with the same respond-then-act idiom as `.shutdown`. `spaces daemon apply-update` (`SpacesCommand.swift`) sends it directly — it is not a `TerminalServiceProfileCommand` — and fails immediately with a not-running error when the socket is absent, rather than starting or retrying the daemon. The Linux installer is its only caller today: `write_linux_install_script` prefers poking the already-running daemon over `systemctl --user restart`, since the poke preserves every session across a reinstall while a systemd-triggered restart would just respawn a fresh process with none of them; a first install, a daemon that is not running, and an older daemon that predates the command all fail the poke and fall back to `systemctl --user restart`.
- Triggers differ per platform because the two platforms stage updates differently. On macOS, `AppKitController.maybeRequestSilentDaemonHandoff` fires `requestDaemonRestart()` silently the moment a fresh `TerminalServiceDaemonStatus` shows a compatible daemon reporting an older app version (`daemonUpdatePending`) — deduped for the app's lifetime by `deviceID` plus target version, so a refused or failed handoff is not retried on every subsequent status refresh. Linux has no equivalent silent trigger, because nothing on the Mac stages a new release into a remote Linux install directory on its own; the poke instead rides the installer, run explicitly by the user or by the paired-recovery SSH flow.
