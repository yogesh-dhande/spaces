...
- Remote host connection runs strict known-host SSH first, probes the remote OS and architecture, selects an exact signed GitHub Release artifact for the current Spaces version, installs the managed daemon under `~/.spaces/compute-hosts/<host-id>/daemon/releases/<version>/`, stores the generated daemon auth token in Keychain, saves the returned certificate fingerprint, validates the direct pinned-TLS `spacesd` endpoint, checks mobile terminal credential readiness, and saves the host only after validation succeeds.
- Remote Hosts shows a setup checklist only when a required setup check fails. The checklist stays visible while the user fixes SSH, OS, tools, install-root, Git, port, daemon, certificate, pinned-TLS, or mobile credential issues outside Spaces. Failed rows include the remote host, the command or check that failed, and a concise fix hint. The panel provides Retry, Copy Diagnostics, and Open SSH Command actions.
- Remote host setup supports macOS 14+ universal remotes and Ubuntu 24.04 Linux remotes on `x86_64` and `arm64`. Source checkouts and source builds are not required on the remote host.
- Saved remote hosts expose Check, Reinstall, Upgrade, Edit, and Uninstall actions. Check reports pinned-TLS reachability, daemon version, managed artifact version, certificate pin state, active session count, and upgrade state. Reinstall reruns the managed remote setup path for the current Spaces build over SSH, revalidates pinned TLS and mobile credential readiness, and resaves the host only after validation succeeds; when the saved daemon is reachable, Reinstall requires it to be idle and waits for shutdown before setup, and otherwise relies on SSH setup to require the saved port to be free. Upgrade requires an idle daemon; when remote sessions are active, Spaces presents a dialog that lists the active sessions and offers Stop Session(s) and Upgrade or Cancel. Accepted upgrades stop the active sessions through the normal runtime lifecycle paths, shut the daemon down through the pinned-TLS admin channel, wait for the listener to exit, install the current managed artifact atomically, relaunch, and revalidate pinned TLS and mobile credential readiness. Uninstall requires an idle daemon when the daemon is reachable, stops it through the pinned-TLS admin channel, waits for exit, runs SSH cleanup even when the daemon is already unreachable, removes managed daemon binaries, symlinks, and uploads, and removes the saved host and Keychain token while preserving workspace roots and remote worktrees.
...
- The spacesd daemon should start the first-party mobile bridge on launch, keep the stable default port reserved while it is running, fall back to a persisted profile-specific stable port if the default is already owned by another Spaces profile, and advertise the bridge to nearby iOS clients with Bonjour.
- The paired iOS client should expose a compact `Launch Spaces on Mac` toolbar action. The action asks the reachable spacesd bridge to launch `SpacesApp` for the current profile only when no live app-owner lease exists; if the app is already running, the action succeeds without restarting the app, stopping sessions, or restarting the spacesd daemon.
...
- Native Spaces terminal windows should attach to an existing service-owned session without restarting the shell. Local-session windows subscribe to the daemon-owned session state stream, and remote-session windows use the owning daemon endpoint for state refreshes and control requests while preserving owner handoff compatibility, final Ghostty renders, and metadata synchronization.
...
- The first-party iOS client should pair with Mac `spacesd`. Mac `spacesd` owns pairing, auth, overview, workspace mutations, and provisioning of remote terminal credentials. Remote compute-host admin tokens are Mac-side secrets and should not be exposed in mobile payloads or persisted by iOS.
- Remote iOS terminal sessions should use direct pinned-TLS access to the owning remote `spacesd` with the scoped terminal credential provisioned for that iOS installation. The Mac bridge remains the path for local Mac sessions, workspace mutations, and remote operations outside terminal state/control/link-preview/chunk reads.
...
- When the GUI creates a Git workspace with an explicit directory name, it asks for confirmation before replacing an existing unregistered Spaces-managed worktree folder at that directory. If the directory name is left blank, Spaces chooses a generated non-conflicting managed directory instead of prompting.
