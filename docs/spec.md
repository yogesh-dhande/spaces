...
- Restart (in the app, on iOS, or `spaces workspace restart`) is a full stop followed by a fresh launch. The stop ends every tracked runtime, ad hoc terminals and coding-agent sessions included. What happens to its panes and tabs is in [Panes and tabs across a stop or restart](#panes-and-tabs-across-a-stop-or-restart). The workspace reads as running for the whole restart, on every client, so nothing that reacts to a stop fires partway through; a restart whose relaunch fails leaves it stopped.
...
- A workspace restart, from any client, keeps the workspace's tracked browser tabs open, since the relaunched services come back on the same URLs. Ad hoc terminal and coding-agent panes close with their sessions.
- A restart run on this Mac through the CLI or MCP keeps each configured process's pane in place (tab, split position, window) for its replacement session: a process whose pane was closed before the restart gets a fresh unselected pane, and a process whose relaunch fails has its held pane closed. A restart from the Mac app or the iPhone closes those panes for a workspace on this Mac, and can close them for a paired device's workspace.
- When another client stops or deletes a workspace, whether it lives on this Mac or a paired device, the Mac closes its panes and its tracked browser tabs once it sees the change. With no Mac app running at the stop, the tabs stay open, because only the app tracks them; deleting that workspace later, with the app running, closes them.
