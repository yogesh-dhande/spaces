...
- Pairing a remote device for macOS asks for the SSH host, with the optional username (defaults to the SSH login) and port (defaults to 22) kept under an "Advanced" disclosure, validates SSH with `BatchMode=yes` and `StrictHostKeyChecking=yes`, requires a remote Mac to have the DMG install markers, prepares supported Linux hosts automatically over SSH when Spaces is missing or not responding, runs `~/.spaces/bin/spaces pair --json` on the remote device, then pairs with the Device API at the effective OpenSSH `HostName` and returned API port. Missing or failing SSH is a setup failure for pairing, remote terminal attach, browser forwarding, and editor opening.
