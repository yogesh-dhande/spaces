...
- The Mac user settings open as a floating dialog window with a two-panel layout: a header bar with the Settings title and a close control, a left navigation list of General, Shortcuts, and Devices sections, and the selected section's content in the right panel. The dialog floats over the main window, leaving the current sidebar selection and detail pane untouched. The General section holds the preferred editor and window focus pulse controls, and the Shortcuts section holds the keyboard shortcut editor.
