...
- Adding a local project imports an existing `spaces.yaml` from that directory as the project template. Adding a Git URL creates the default worktree, then imports `spaces.yaml` from that worktree when the file exists; invalid YAML prevents project creation and rolls back the managed clone and worktree.
- Export writes the active project template as a full canonical `spaces.yaml`, overwriting the file at the project config path. Import opens a preview with Apply, Dismiss, and an unchecked `Update all workspaces` checkbox. Dismiss leaves project and workspace state unchanged. Apply updates the project template; the checkbox also updates every workspace, including archived workspaces.
- Saving project settings edits only the project template. It does not rewrite any existing workspace settings, including the default workspace.
