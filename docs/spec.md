...
- The selected workspace's right panel is a tabbed terminal panel scoped to that workspace, and nothing else: the tab strip at the top, the selected tab's panes filling the rest edge to edge, and the workspace footer strip at the bottom. Each tab holds one or more panes; every pane hosts a terminal session (future content kinds may join). A terminal session has at most one pane anywhere — opening an already-open session focuses its existing pane instead of duplicating it.
- Each pane shows a compact flat header — session title, split-right and split-down buttons, and a close button — with no card border or outline around the pane; the focused pane reads from its tinted header. Splitting opens the command palette in a session-picker mode listing `New terminal session` first and then existing sessions in scope; the chosen session fills the new pane, moving it from any pane it already occupied. Pane and tab sizes from divider drags persist.
...
- The sidebar and the right panel each own a footer strip; neither shows keyboard-shortcut hints. The sidebar footer holds the Alerts entry (bell icon, title, and unread badge). The right panel footer shows the selected workspace's details in one compact row: a run-state dot, the workspace name (its git branch, or the project folder name for non-git; read-only — renaming a git workspace's branch changes the name), a runtime warning icon when something needs attention, the git branch when one is recorded, the directory path, and the notes, launch/restart, stop, and overflow actions. The footer stays with the workspace across the panel, loading, and setup views, and clears when no workspace is selected.
