...
- Every visible workspace row shows a compact vertical list of its runtime targets beneath it — browser sessions, configured processes (running or not), ad hoc terminals, and coding agents — in the same order the numbered window shortcuts and window cycling use. Each target row shows a kind icon tinted by run state (running, exited, or not started) and the target name. When the workspace is selected, target rows also show their `⌘<number>` shortcut chips. A non-git project's flat row lists its single workspace's targets the same way.
...
- The selected workspace's right panel is a tabbed terminal panel scoped to that workspace. Each tab holds one or more panes; every pane hosts a terminal session (future content kinds may join). A terminal session has at most one pane anywhere — opening an already-open session focuses its existing pane instead of duplicating it.
- Each pane shows a compact header with the session title, split-right and split-down buttons, and a close button; the focused pane carries an accent border. Splitting opens the command palette in a session-picker mode listing `New terminal session` first and then existing sessions in scope; the chosen session fills the new pane, moving it from any pane it already occupied. Pane and tab sizes from divider drags persist.
- The `New terminal` shortcut and the tab strip's `+` button start a fresh ad hoc terminal session and open it as a new tab in the selected workspace's panel.
...
- Right-clicking a sidebar target row shows a context menu with Start, Stop, and Restart (each shown only when the action applies to the target's current state), Rename, and Open in New Window (not yet enabled). Rename edits the name inline in the row; Return saves and Esc cancels. Renaming an ad hoc terminal renames its session; renaming a configured process, coding agent, or browser session renames its workspace configuration entry, so a running process picks the new name up on restart.
...
- The workspace settings dialog matches the project settings dialog chrome: a header with the project and workspace name and a close control, the directory path at the top of the body, then the workspace's configuration sections for Browser sessions, Processes, Coding agents, Named ports, and Stop script. Each section shows its configured items as rows and expands inline into an edit form when the pencil icon is clicked; the `+ add` header button appends a draft item; every edit saves immediately. The dialog is configuration-only: sections show edit and delete controls but no run, stop, restart, or focus actions — runtime control lives on the sidebar target rows.
- The workspace detail pane shows the workspace name (its git branch, or the project folder name for non-git), launch/restart, stop, and overflow actions, the directory path, the git branch name when one is recorded, and the inline notes editor. The workspace name is read-only and is not renamed directly; renaming a git workspace's branch changes the name. A `⋯` overflow button in the action row exposes Copy path and Reveal in Finder, with Reveal in Finder available as a keyboard-invokable menu item via `⌘⇧F`.
...
- Sidebar runtime rows and Alerts reflect terminal title and exit-state updates from the local Mac and paired remote devices without requiring app reactivation.
...
- Opening or focusing a native terminal window in owner-seeking mode should express ownership intent and may transfer ownership from another attached client, including a client on another device, without requiring a separate confirmation. This applies to app-driven opens, focus requests, CLI `terminal show`, and iOS terminal detail opens.
- Owner-seeking terminal opens should not mount a passive live terminal surface. While the window is not the active owner it should show takeover and status UI only, and once it becomes owner it should render the current terminal state without restarting or recreating the shell session. The explicit Take Over action remains the user-facing control for a window that is already attached as a viewer.
...
- Ad hoc built-in terminal windows opened from Spaces, including the `New terminal` shortcut path, should remain listed in the sidebar's runtime-target list even before their native yabai window ID has been backfilled.
...
- Adding a named port from the workspace settings dialog should reserve its port number immediately instead of waiting for the next workspace launch.
