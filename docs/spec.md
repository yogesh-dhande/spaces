...
- iOS row taps should open a live terminal when the row has a session. Rows without a live session should route the primary action to running the configured process or coding agent when that action is available.
...
- The iOS client should let users open a persistent workspace terminal rooted at the workspace directory, and run, stop, or restart configured processes and coding agents through the paired Mac bridge.
