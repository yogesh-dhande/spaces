...
- `spaces device pair` pairs the CLI's profile with another device's daemon over SSH (`--ssh user@host`, with automatic Linux daemon preparation) or by redeeming a `spaces://pair` link (`--link`). `spaces device list` shows paired devices and `spaces device remove` deletes a pairing and its stored credential. The CLI shares the Mac app's per-profile client identity, paired-device records, and credential store, so devices paired from either surface are usable from both.
...
- The CLI should expose `spaces project list`, `spaces workspace list`, `spaces workspace create --project <id> --branch <branch>`, `spaces workspace start --workspace <id>`, `spaces workspace restart --workspace <id>`, `spaces pair`, `spaces pair --json`, `spaces agent signal --workspace <id> --session <terminal-session-id> <event>`, `spaces terminal list`, `spaces device pair`, `spaces device list`, `spaces device remove`, and `spaces mcp`.
