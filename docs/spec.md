...
- The selected workspace's right panel is a tabbed terminal panel scoped to that workspace, and nothing else: one flat tab strip at the top (running to the top of the window — the main window shows no titlebar or window title; the app identity row lives in the sidebar footer), the selected tab's panes filling the rest edge to edge, and the workspace footer strip at the bottom. Each tab holds one or more panes; every pane hosts a terminal session (future content kinds may join). A terminal session has at most one pane anywhere — opening an already-open session focuses its existing pane instead of duplicating it.
- Tabs render flat — no pill or chip outline — with the selected tab marked by full-color text and an accent underline; each tab has a close glyph. Clicking a tab selects it and returns keyboard focus to the pane that most recently held focus in that tab. Right-clicking a tab offers Rename, which edits the tab's name in a small popover (Return saves, Esc cancels); a custom tab name persists with the layout and survives relaunch, and clearing it returns the tab to its derived title (its first pane's target). Tabs can be named independently of their panes' sessions, which matters once a tab holds several panes.
- Panes carry no chrome of their own — no header, border, or outline; the terminal surface fills the pane. Pane and derived tab titles use the runtime target's name (the same name the sidebar target row shows, e.g. `codex` or `npm:dev`), not the terminal's own window title. The tab strip's right side holds the pane actions: split right, split down (both act on the selected tab's focused pane), and new tab. Splitting opens the command palette in a session-picker mode listing `New terminal session` first and then existing sessions in scope; the chosen session fills the new pane, moving it from any pane it already occupied. Pane and tab sizes from divider drags persist.
