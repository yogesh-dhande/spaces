...
- The iOS client should be able to browse all live terminal sessions, auto-attempt ownership takeover when opening one, render one session at a time through a Ghostty render-frame backed terminal view only after ownership is acquired, and keep ended-session final renders available when Ghostty state was persisted.
...
- The iOS terminal viewport should size to the visible render area above the software keyboard and accessory toolbar so bottom rows and the prompt remain visible with long terminal content.
