...
- iOS owner scrollback history should be seeded from bounded raw output only when the complete replay fits the transfer budget; oversized terminal logs should keep the live bootstrap snapshot and incremental output instead of replaying an unsafe partial VT stream.
...
- The iOS terminal keyboard accessory should use a compact fixed-height toolbar: the scrollable key strip is ordered `tab`, `/`, `~`, `|`, `-`, `_`, `esc`, `ctrl`, iPhone uses tighter button widths and spacing than iPad, and a single arrow-key joystick plus the keyboard hide/show control stay pinned on the trailing edge.
