...
- The selected workspace's right panel is a tabbed terminal panel scoped to that workspace, and nothing else: one flat tab strip at the top (running to the top of the window — the main window shows no titlebar or window title; the app name sits next to the logo in the sidebar), the selected tab's panes filling the rest edge to edge, and the workspace footer strip at the bottom. Each tab holds one or more panes; every pane hosts a terminal session (future content kinds may join). A terminal session has at most one pane anywhere — opening an already-open session focuses its existing pane instead of duplicating it.
- Tabs render flat — no pill or chip outline — with the selected tab marked by full-color text and an accent underline; each tab has a close glyph. Right-clicking a tab offers Rename, which edits the tab's name in a small popover (Return saves, Esc cancels); a custom tab name persists with the layout and survives relaunch, and clearing it returns the tab to its derived title (its first pane's session). Tabs can be named independently of their panes' sessions, which matters once a tab holds several panes.
- Panes carry no chrome of their own — no header, border, or outline; the terminal surface fills the pane. The tab strip's right side holds the pane actions: split right, split down (both act on the selected tab's focused pane), and new tab. Splitting opens the command palette in a session-picker mode listing `New terminal session` first and then existing sessions in scope; the chosen session fills the new pane, moving it from any pane it already occupied. Pane and tab sizes from divider drags persist. The focused pane's identity lives in the workspace footer (see below), including its close-pane control.
...
- The sidebar's Alerts entry sits at the top of the sidebar (bell icon, title, shortcut hint, and unread badge). The right panel owns a footer strip showing the selected workspace's details in one compact row: a run-state dot, the workspace name (its git branch, or the project folder name for non-git; read-only — renaming a git workspace's branch changes the name), a runtime warning icon when something needs attention, the git branch when one is recorded, the directory path (selectable, so it can be copied with `⌘C`), the focused pane's title with a close-pane control, and the notes, launch/restart, stop, and overflow actions. The footer stays with the workspace across the panel, loading, and setup views, and clears when no workspace is selected.
