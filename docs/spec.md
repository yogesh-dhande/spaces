...
- The folder source is a path text field with directory autocomplete, not a native file picker, so projects can be created by path on the local Mac or on a selected remote device. As the user types, suggestions come from the active device's filesystem; on commit the active device validates the path and loads any `spaces.yaml` before the project is saved.
...
- Sidebar workspace rows show the workspace title; the git branch name is surfaced in the workspace detail pane rather than the sidebar to keep rows compact.
- Sidebar workspaces under a project are ordered with the default workspace first, then the rest sorted alphabetically by title using natural, case-insensitive comparison, so the list is easy to scan.
- The workspace detail pane is a single scrollable page: title + actions at the top, directory path with copy and reveal-in-Finder buttons, the git branch name when one is recorded, inline notes editor, then configuration sections for Processes, Browser sessions, Coding agents, Named ports, and Stop script. The workspace title renames inline: double-click or right-click the title and choose Rename, edit in place, then press Return to save or Esc to cancel. Each section shows its configured items as rows and expands inline into an edit form when the pencil icon is clicked; the `+ add` header button appends a draft item. Running process and coding-agent rows should expose stop and restart actions before edit and delete, while non-running configured rows should show run before edit and delete. Named-port rows should show the reserved port number as secondary text next to the configured name. A `⋯` overflow button in the action row exposes Copy path and Reveal in Finder, with Reveal in Finder available as a keyboard-invokable menu item via `⌘⇧F`.
...
- The iOS client should let users create a workspace in any existing project on the active paired daemon. Git projects should expose title, create-branch or existing-branch mode, branch name, base branch, optional directory name, and daemon-local source; non-git projects should hide branch fields.
...
- For git projects, new workspaces are branch-oriented and should support an existing-branch picker, a new-branch entry path, base branch, directory name, title, and notes inputs.
