...
- Explicit built-in terminal window close is a stop request.
  Closing a process, coding-agent, or ad hoc Spaces terminal window stops the runtime that owns that session. Losing track of a window without an explicit close remains a recoverable runtime-health issue rather than an implicit stop.
...
- Closing a built-in terminal window is an explicit stop request for the owning runtime: process terminals stop the process, coding-agent terminals stop the agent session, and ad hoc workspace terminals terminate their service-owned terminal session. Stopping a workspace also terminates ad hoc built-in terminal sessions owned by that workspace.
...
- Closing a process terminal window is an explicit stop request for that process. If a tracked process window becomes stale outside explicit user close handling while the process remains alive, Spaces should recover the terminal view and reattach when the user opens a new window for it.
