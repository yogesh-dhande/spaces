...
- Pairing a remote device for macOS asks for SSH host, user, and port, validates SSH with `BatchMode=yes` and `StrictHostKeyChecking=yes`, runs `~/.spaces/bin/spaces device pair --json` on the remote device, then pairs with the Device API at the effective OpenSSH `HostName` and returned API port. Spaces never installs the remote daemon itself: when Spaces is not installed on the remote device, pairing fails with install instructions — an Ubuntu 24.04 device is given a version-pinned installer command to run there, and a Mac is directed to install the Spaces app. Missing or failing SSH is a setup failure for pairing, remote terminal attach, browser forwarding, and editor opening.
