...
- A Spaces-owned ad-hoc built-in terminal should appear in `Coding Agents` while its live foreground process is a known coding-agent command such as `codex`, `claude`, `claude-code`, or `opencode`. When the foreground process is a shell or an unknown command, that terminal should appear with the other terminal rows under `Processes`.
- A signal-established ad-hoc agent row should remain in `Coding Agents` only while the terminal foreground process matches the process identity present when the signal established or updated the row. If no foreground process has been sampled yet, the row may remain until the first sample; a non-shell first sample becomes the row's identity baseline. If the foreground process changes to a shell or unrelated command, the terminal should lose ad-hoc agent status even if earlier signal events were recorded.
