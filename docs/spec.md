...
- Sidebar rows show their nesting through a leading glyph and indentation. A project row leads with a project-type glyph — a commit-graph mark for a git repository, or a folder tinted by run state (green running, muted stopped) for a non-git directory. A git project's workspace rows indent one level under it and lead with a run-state status dot; their runtime-target rows indent a further level and lead with their `⌘`-number chip slot. A non-git project's runtime-target rows indent one level under the project row, which stands in for the missing workspace level.
