...
- **Reversible** — terminal session and pane lifecycle (`start-workspace-terminal-session`, `terminate-terminal-session`, `open-workspace-terminal`, `close-terminal-session-window`, `focus-terminal-session-window`, `terminal-window-shortcut`, `terminal-service-control`, `clear-workspace-agent-windows`), workspace runtime (`stop-workspace`, `run-workspace-process`, `stop-workspace-process`, `restart-workspace-process`, `launch-workspace-agent`), the workspace settings a sweep drives and then clears (`set-workspace-agent-launchers`, `set-workspace-browser-session-urls`), focus, cycling and window visibility, desktop input synthesis, and `mobile-status`.
- **Refused** — anything that creates or removes projects and workspaces (`seed-fixture`, `register-project`, `create-workspace`, `cleanup-fixtures`, `stop-fixtures`, `archive-workspace`, `hide-workspace`, the `e2e` lanes, `record-mobile-demo`), overwrites configuration with no record of what was there (`set-workspace-stop-script`, `add-workspace-process`, `remove-workspace-process`), changes device pairings (`open-device-pairing-window`, `open-remote-device-pairing-window`, `pair-remote-device`, `seed-paired-device`), carries an unclassifiable Device API request (`mobile-serve`, `mobile-request`, `service-tunnel`), acts on the machine's profile inventory (`profile`), or fabricates a failure in the user's own app (`show-window-issue-modal`).
