...
3. Promote it: `gh release edit <tag> --prerelease=false --latest`, or untick "Set as a pre-release" and tick "Set as the latest release" together in the GitHub releases UI. Both matter — `releases/latest` is what the stable feed and `install.sh` resolve through, and clearing the pre-release flag alone leaves it wherever it was last pinned. The `released` event then runs [`release-promote.yml`](../.github/workflows/release-promote.yml), which checks out the release's commit, confirms `releases/latest` resolves to the promoted tag before building anything, redeploys the website, and confirms `https://usespaces.dev/releases/appcast.xml` serves the promoted version. Releases created by the Actions token do not raise `released`, so the tag build cannot trigger this workflow.
