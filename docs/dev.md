...
That selector is the only route from this tooling to the installed profile, and it is an argument rather than an environment variable so an inherited shell binding can never reach live state — `SPACES_DB_PATH` stays refused inside `~/.spaces/` and `~/.spaces-dev/profiles/` for every process. A bound invocation serves the installed profile whole: both profile overrides are dropped, so its sockets and session directories cannot land under another profile's runtime root, and nothing it launches is handed either override — not a daemon it has to start, not the shell of a terminal session it opens, not a workspace stop script — so no child can resolve a different profile from the one it was started for. The `spaces` CLI has no counterpart, because there the same surface would be product API.
