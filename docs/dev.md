...
### QA of the installed build from `spacese2e`

`spacese2e` never ships to users, so it is where scripted QA of the installed build lives. Passing `--installed-profile` before a command points that one invocation at the installed profile (`~/.spaces`) instead of the checkout's development profile:

```bash
apps/macos/.build/debug/spacese2e --installed-profile profile-show
apps/macos/.build/debug/spacese2e --installed-profile surface-snapshot --spaces-pid <pid>
apps/macos/.build/debug/spacese2e --installed-profile terminate-terminal-session <session-id>
```

That selector is the only route from this tooling to the installed profile, and it is an argument rather than an environment variable so an inherited shell binding can never reach live state — `SPACES_DB_PATH` stays refused inside `~/.spaces/` and `~/.spaces-dev/profiles/` for every process. A bound invocation serves the installed profile whole. The entire `SPACES_*`/`SPACESD_*` environment namespace is dropped — both from what it acts on and from what it hands anything it launches: a daemon it has to start, the shell of a terminal session it opens, a workspace stop script. The namespace rather than a list, so a variable added later is covered on the day it is introduced; the E2E shells a sweep is usually run from export several of them. So its sockets and session directories cannot land under another profile's runtime root, no child resolves a different profile from the one it was started for, a sweep cannot start that checkout's `spacesd` against `~/.spaces`, and it cannot leave the user's installed daemon bound to an ephemeral Device API port their paired devices can no longer reach. The user's own shell configuration — `PATH`, `SHELL`, `TERM`, `ZDOTDIR` — is inherited untouched, so a terminal it opens behaves like the user's terminal. `HOME` is corrected rather than dropped, since a child resolves the installed profile as `<home>/.spaces` and cannot see the parent's binding: without that, a sweep run from a lane exporting a temporary home would start a daemon on a throwaway profile and wait for it on the account profile's socket. "Installed profile" means the account's own `~/.spaces`, read from the password database rather than `HOME`, so a lane that exports a temporary home cannot make a sweep report readings it took from a throwaway profile. It is also refused any argument naming a path inside `~/.spaces`: a permitted command owns the destination it is given — `record-screen` deletes and recreates its `--output` and `--ready-file`, `dump-terminal-session-window-state` overwrites its `--output-path` — so a path in there would destroy live state under a classification that promises it cannot. The `spaces` CLI has no counterpart, because there the same surface would be product API.

Every command declares its installed-profile access in one table, `SpacesE2EInstalledProfileAccess.byCommandName`, and the selector is refused for anything that table does not permit:

- **Read-only** — `surface-snapshot`, `dump-workspace`, `focusable-window-names`, `lookup-workspace`, `terminal-service-state`, `record-screen`, `render-update-text`, `mac-client-installation-id`, and the `profile-show` / `profile-app-owner` / `profile-socket-paths` / `profile-desktop-control-owner` / `profile-wait-for-desktop-control` lookups. Read-only is a claim about the whole run: these take no destination inside the profile root (refused for every command), and none of them lazily initialises anything on its read path — `dump-workspace` reads workspace settings without the seeding the ordinary accessor performs, so a workspace that has none is reported as having none rather than given some.
- **Reversible** — terminal session and pane lifecycle (`start-workspace-terminal-session`, `terminate-terminal-session`, `open-workspace-terminal`, `close-terminal-session-window`, `focus-terminal-session-window`, `terminal-window-shortcut`, `terminal-service-control`, and `dump-terminal-session-window-state`, whose refresh attaches as the session owner unless `--viewer` is passed), workspace runtime (`stop-workspace`, `run-workspace-process`, `stop-workspace-process`, `restart-workspace-process`, `launch-workspace-agent`), focus, cycling and window visibility, desktop input synthesis, and `mobile-status`.
- **Refused** — anything that creates or removes projects and workspaces (`seed-fixture`, `register-project`, `create-workspace`, `cleanup-fixtures`, `stop-fixtures`, `archive-workspace`, `hide-workspace`, the `e2e` lanes, `record-mobile-demo`), overwrites configuration with no record of what was there (`set-workspace-stop-script`, `add-workspace-process`, `remove-workspace-process`, `set-workspace-agent-launchers`, `set-workspace-browser-session-urls`), deletes rows belonging to something still running (`clear-workspace-agent-windows`), changes device pairings (`open-device-pairing-window`, `open-remote-device-pairing-window`, `pair-remote-device`, `seed-paired-device`), carries an unclassifiable Device API request (`mobile-serve`, `mobile-request`, `service-tunnel`), acts on the machine's profile inventory (`profile`), or fabricates a failure in the user's own app (`show-window-issue-modal`).

A command is permitted when it only reads, only undoes or re-does something the sweep itself created, or does exactly what a user can already do to that profile from the product's own UI — which is why `stop-workspace` is permitted (the app offers it, and losing the contents of the terminals it closes is inherent to stopping a workspace) while the setters are not. Nothing in this tooling captures a workspace's configured launchers or browser URLs before replacing them, so "the sweep clears it again afterwards" restores nothing; `launch-workspace-agent` stays permitted, so an already-configured launcher's lifecycle is still testable there. `clear-workspace-agent-windows` is refused for the neighbouring reason: it deletes every coding-agent row in a workspace, including rows it did not create, while those agents' terminals stay live — a state the product's own UI never produces.

Creation is refused because nothing in the product removes a project or a workspace: one created on the installed profile stays in the user's list for good, so workspace lifecycle work belongs on a development profile. A command with no entry in the table is refused as well, so a command added later states what it does to a live profile before it can reach one. Each refusal prints the command, the reason its classification carries, and that dropping the selector runs it against the development profile instead.

A bound invocation is a guest on that profile in two further ways, both because a checkout is normally ahead of the installed release. It starts no daemon of its own build for the installed profile — only `~/.spaces/bin/spacesd` or `/usr/local/bin/spacesd` may serve it, so a sweep that stopped the installed daemon gets `daemonNotFound` rather than a development `spacesd` on the user's database. And it never upgrades that database's schema: a command that opens the store directly refuses when the installed profile's schema is behind the checkout's. The refusal does not send you to start the installed app — in the case that produces it, that build is older than the checkout and cannot produce the newer schema — it asks for an installed release that includes the schema this build needs, or for the command to be run without `--installed-profile`. Commands that go through the daemon's socket — the snapshots, `terminal-service-state`, `terminal-service-control` — are unaffected by that, since they read what the installed daemon serves.
