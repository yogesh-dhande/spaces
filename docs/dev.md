...
Every `scripts/verify.sh` run closes with a lane summary naming the lanes it ran with their results, and what no verify.sh lane covers: the desktop, mobile, and remote-daemon E2E suites, and the Linux daemon unit lane and Linux artifact smoke test, which run in CI. The summary prints on failure as well as success, so a run is read as what it actually covered rather than as everything.
