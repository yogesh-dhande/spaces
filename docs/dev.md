...
That selector is the only route from this tooling to the installed profile, and it is an argument rather than an environment variable so an inherited shell binding can never reach live state — `SPACES_DB_PATH` stays refused inside `~/.spaces/` and `~/.spaces-dev/profiles/` for every process. A bound invocation serves the installed profile whole. Every variable that could redirect it elsewhere — `SPACES_DB_PATH`, `SPACES_RUNTIME_DIR`, and the `SPACESD_EXECUTABLE` daemon pin every terminal E2E lane exports — is dropped both from what it acts on and from what it hands anything it launches: a daemon it has to start, the shell of a terminal session it opens, a workspace stop script. So its sockets and session directories cannot land under another profile's runtime root, no child resolves a different profile from the one it was started for, and a QA sweep run from an E2E shell cannot start that checkout's `spacesd` against `~/.spaces`. It is also refused any argument naming a path inside `~/.spaces`: a permitted command owns the destination it is given — `record-screen` deletes and recreates its `--output` and `--ready-file`, `dump-terminal-session-window-state` overwrites its `--output-path` — so a path in there would destroy live state under a classification that promises it cannot. The `spaces` CLI has no counterpart, because there the same surface would be product API.
