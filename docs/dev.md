...
For the coding-agent orchestration surface (`spaces agent` list/status/brief/subscribe/kill and notification injection):
...
Deploys use `FirebaseExtended/action-hosting-deploy` against project `spaces-a1814`, authenticated with the `FIREBASE_SERVICE_ACCOUNT_SPACES_A1814` secret. Because the site's `prebuild` reads GitHub releases with `gh`, every workflow that builds `apps/web` passes `GH_TOKEN`.

`apps/web/firebase.json` is the only Hosting config: the public directory, headers, and the app rewrite. Every deploy, preview and live, runs the action with `entryPoint: apps/web` so it reads that same file. The Hosting site keeps a release storage limit of five versions (the site config's `maxVersions`, shown in the Firebase console under Hosting, Release history, release storage settings), which keeps stored versions, each carrying both staged Sparkle zips, well under the storage quota.
