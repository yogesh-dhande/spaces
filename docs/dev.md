...
The code pane's web app (`apps/macos/CodePaneWeb`) is a separate Vite/TypeScript project built with npm (see its `README.md`); `npm run build` there emits directly into `apps/macos/Sources/spacesui/Resources/CodePane/`, which is checked in and bundled into the app as a `.copy` resource of the `spacesui` SwiftPM target. Editing that web app requires npm and a rebuild of the checked-in bundle; the Swift build and test lanes above never invoke node.
...
`apps/macos/.build/debug/spacese2e e2e app --scenario code-pane --samples 5` runs the real bundled code pane against local and paired remote workspaces. It covers dirty and live-refreshed diffs, Last Commit/Uncommitted scope changes, Editor rendering, singleton workspace retargeting, and a three-point diff and file-size matrix (about 4 KiB, 128 KiB, and 768 KiB). `--scenario code-pane-streaming` is the focused progressive-diff gate: it asserts that the manifest paints the sidebar before patches, a selected late file is promoted from the queue, a multi-chunk patch completes, and only the new side of a diff can be edited, that an edit autosaves to disk with the session left open, and that Esc ends the session. It also exercises local A→B→A workspace and app-restart recovery for independent diff/editor state, then the no-agent Start Agent and blocked-comment guidance flows using disposable commands and a hook-backed fixture agent. Browser paint latency samples for the manifest, selected-file patch, complete patch set, and inline save are written with the existing p50/p95/max report. RSS, CPU percentage and cumulative CPU time, thread, and file-descriptor samples for the Spaces process tree and the launchd-owned WebKit XPC services in that exact app instance's kernel process coalition are written under `apps/macos/.artifacts/real-system-profiles/`. `--scenario code-pane-churn --samples N` uses an ABBA ordering with N rewrites per arm to compare the Editor closed and visible while concurrent writers repeatedly rewrite large tracked file sets in three local workspaces and two remote workspaces. `--scenario code-pane-soak --duration-seconds 1800` alternates local/remote workspace retargets and the same sizes for the requested soak duration, with post-render RSS-growth and CPU gates. These scenarios require the remote `.env` configuration. Set `SPACES_APP`, `SPACES_CLI`, and `SPACESD_EXECUTABLE` together to equivalent release binaries when measuring production-build behavior; the debug `spacese2e` helper may still drive the lane because it is outside the measured process tree.
...
The lane keeps its Zig and SwiftPM build caches in a per-worktree pair of named Docker volumes, `spaces-linux-zig-<hash>` and `spaces-linux-swift-<hash>`, hashed from the worktree's absolute path. The deploy stamps each volume with the worktree path it built from (`dev.usespaces.spaces.worktree`), and `scripts/prune-linux-e2e-cache-volumes.sh` removes a volume whose labeled worktree no longer exists on disk; a volume predating that label is matched by hash against this clone's `git worktree list` instead. It scopes removal to the `spaces-linux-` name prefix and never uses a blanket Docker prune, and it stops without deleting anything if either inventory cannot be taken. `deploy_linux_spacesd_e2e.sh` runs it on the build path, just before it creates this worktree's pair, so the leak from a removed worktree self-heals on the next deploy that builds, and the `prune-merged` cleanup workflow runs it once at the end of a pruning pass.

Linux daemon-side unit suites (the `#if os(Linux)` tests in `spacesterminalcoreTests`, `spacesterminalghosttyTests`, and `spacesdeviceapiTests`, plus `SpacesTestHostDetectionTests`, which is cross-platform because test-host detection rests on a different signal per platform) run inside Docker via `apps/macos/scripts/run_linux_tests.sh`:
...
`e2e_automations.sh` is a daemon-only test (no app, desktop control, or hotkeys) that binds to the current worktree profile from `spacese2e profile-show --shell`, so it only ever drives this checkout's daemon and never another worktree's; the daemon autolaunches on the first profile command. It registers a fixture project and targets its default workspace through `spacese2e automation-*` — the same profile-socket commands the app's Automations UI sends — as the seam for authoring automations in tests, since there is no `spaces automation` CLI surface for end users. Scenarios a-e use fast fake scripts (`--script`), never a real coding agent: a manual automation whose script writes a marker and exits 0 runs to `succeeded` with the marker in its output and the run's terminal session stamped with `kind=automation`, the run id, and the fixture workspace id; a script that exits 3 runs to `failed` with that exit code; a `concurrency=skip` automation triggered while a run is still sleeping records a `skipped(concurrency)` row for the second trigger, then canceling the sleeping run leaves it `canceled`; a 2-second timeout against a longer sleep lands `timed_out`; and a `SPACES_AUTOMATION_RUN_ID` attribution check confirms the run's own workspace-bound session carries the run id. Scenario f drives what an `agent`-kind (`--kind agent`) run can exercise without a real provider: creating an automation whose workspace id does not resolve is refused outright, and End-agents on a terminal run is confirmed to be an accepted no-op that leaves it `failed`. Scenario g is the real-provider lane: it registers its own fixture workspace, creates an agent automation running `claude --model haiku` with a three-line prompt, and asserts the agent answered it, that every line of the prompt reached the agent, that the prompt was submitted exactly once (no re-send duplicated the work), and that the run stamped `prompt_delivered_at`. It is environment-gated the way the agent-kind exclusion is: with no `claude` on `PATH` it prints a skip line and the lane still passes, and it also skips when the agent's screen shows a folder-trust dialog, since a provider that is not authenticated or not trusted for the fixture path cannot answer anything. Running it therefore needs a signed-in `claude` and costs a real (haiku-sized) model call. The remaining agent-kind lifecycle — foreground detection, `done` completion, ending a lingering live agent — is covered by `workspacecoreTests/AutomationServiceTests`, because a fixture script's foreground process resolves to its shebang interpreter, never the agent name it is disguised as, so only a real provider binary can be detected. The script deletes every automation it creates on exit, which also cancels any still-running run and cleans up its artifacts.
...
Eleven scenarios run under each of three shaped network profiles, both directions:
...
The scenarios: `cold-open`, `cold-open-owned` (the same cold open, but against a session a native Mac window already owns, opened with `spaces terminal show` before the UI test runs; local-only, since no Mac window can own a remote session, so a `--remote` run leaves it out of the default set and rejects it when asked for by name), `back-and-forth`, `keyboard`, `streaming`, `scrollback`, `background-terminal`, `background-list`, `reconnect` (the only iOS scenario that scripts a dead link, through the shaping proxy's control port), `idle`, and `mac-reconnect`. `spaces terminal show` requires a running Mac app holding this profile's app-owner lease, so a `cold-open-owned` run stages this worktree's Mac app when no instance for the profile is running and quits it at the end of the run; an instance that was already running is left alone.

`mac-reconnect` measures the same link-down/link-up procedure on the Mac client's paired-device terminal pane, so a change to that pane's redial behavior has before and after numbers. It runs against a throwaway daemon of its own, started on an ephemeral profile under the run root (`SPACES_DB_PATH=<run root>/upstream/spaces.db`) with one registered git fixture project, whose workspace gets its own sidebar row: the Mac app shadows a paired device whose project ids duplicate ones it already shows (the sidebar merges every device's projects first-wins by id, local section first), so a device serving the app's own daemon renders an empty section. The lane pairs the Mac app's installation id with that daemon, seeds the shaping proxy's loopback endpoint as a paired device, launches this worktree's `SpacesApp` with the performance log path set, opens the seeded device's copy of the fixture session through accessibility automation, and drives the link through the proxy's control port while reading the pane's own `connection_stage` and `stream_first_frame` events. The iOS `reconnect` scenario brings the link back with `link up`, which closes the connections the outage tainted, while `mac-reconnect` uses `link up dead`, which restores the link for new connections and leaves the tainted ones black-holed, so the pane sits out its own redial budget the way it does on a path change (NAT rebinding, VPN or mesh route change); it brings the link back once the pane reports a stream dial in flight (`stream_dial_begin`) and the proxy has accepted every connection that attempt opens into the dead link, so every profile measures the same point of that budget. Each profile shapes one daemon at a time: the iOS scenarios against the run's target daemon, then `mac-reconnect` against its own upstream. The app is launched and quit per profile; the seeded device record and the throwaway daemon go away with the run, and this worktree's profile daemon is left alone. The scenario refuses to borrow a Mac app instance that is already running for the profile, since only an app it launched itself writes the performance log. Its numbers land in the report's "Mac reconnect" table, whose "stranded dials" column counts the connections the outage left parked in the dead link, counted only when the proxy also accepted the recovering dial afterwards and that dial belongs to a later connect attempt than the parked one; the scenario fails when the count is zero, so a run where the link came back in time to rescue the dial it was timing cannot report a recovery number.
...
Each run gets its own root under `~/.spaces-dev/ios-baseline/<UTC timestamp>/`, holding the client performance log (`device-perf.jsonl`, iOS and Mac app events plus lane markers written by the UI test and the runner), the shaping proxy's log (`shaper.jsonl`, byte accounting, connection lifecycle, link state, profile), scenario bookkeeping (`sessions.json`), one `xcodebuild-<profile>-<scenario>.log` per scenario, `runner.log`, and the rendered `report.md`. To compare before and after a change, run the lane in two worktrees (or two commits of the same worktree) with the identical flags and read the two `report.md` files side by side; the lane itself has no compare mode and stores no baseline.
...
The pre-commit hook runs `scripts/verify.sh`, which formats staged macOS Swift source and test files, lints, builds, then runs the SwiftPM coverage tests and the iOS tests together.
...
QA of a released build runs the installed `/Applications/Spaces.app` against the QA profile: a throwaway profile at `~/.spaces-dev/qa`, seeded from a copy of `~/.spaces`, so a sweep can create, mutate, and delete whatever it needs while the installed profile, its daemon, and its database stay untouched.

```bash
apps/macos/.build/debug/spacese2e qa-profile create
apps/macos/.build/debug/spacese2e qa-profile launch
apps/macos/.build/debug/spacese2e qa-profile deploy-remote   # the Linux daemon half, when a device is configured
apps/macos/.build/debug/spacese2e qa-profile stop
apps/macos/.build/debug/spacese2e qa-profile remove
```

`create` copies the installed daemon database and client database through SQLite's online backup API, which is safe while the installed daemon is serving them, and strips from the copies what the QA profile must not inherit: paired devices, terminal sessions with the coding-agent, client, attachment, and remote-subscription rows keyed by them, running processes, runtime targets, automation runs that were queued or still executing, and the router port the installed daemon's own router holds. Device identity, TLS material, auth tokens, and the runtime directory are not copied at all, so the QA profile generates its own. Projects, workspaces, their settings and ports, and automations come across, which is what makes the sweep realistic; automations arrive disabled, because the copied workspaces name real repository directories, and the QA daemon derives its own router port rather than reusing the installed one. Unfinished workspace setup resolves to succeeded, since nothing in the QA profile finishes a setup the installed daemon was running and the value meaning "not run yet" is the one that makes a daemon run the script itself. The client copy also loses its browser-session window ids, its terminal owner client ids, and its editor state, each of which names something only the installed profile owns: windows on the user's desktop, attachments in the installed daemon's database, and unsaved buffers that autosave into the user's own repository. It refuses when `~/.spaces-dev/qa` already exists, so `remove` comes first, and it refuses a snapshot whose schema version is higher than the checkout knows, because a newer installed build can hold live-state tables the strip cannot name: run the lane from a checkout at or ahead of the installed build. An older installed build is fine, since the strip names only the tables the snapshot holds.

`launch` starts the installed app's own executable with `SPACES_DB_PATH` naming the QA database, set only in the launched process's environment and never exported into a shell, and with `_` naming that executable, since daemon resolution reads `_` first and an inherited one would point the installed app at a `spacesd` beside the helper that ran the lane, then waits for the QA profile's app-owner lease to name the launched pid. That app starts the `spacesd` bundled beside it and never the LaunchAgent job, which belongs to the installed profile alone. Its output goes to `~/.spaces-dev/qa/qa-app.log`. An inherited project or workspace in the QA profile keeps the real directory it always named, so it is material to read and measure rather than to delete, start, or restart: create, delete, and restart passes belong on a fixture project the QA run adds for itself. Because a launched binary inherits the shell it is launched from, `launch` and `deploy-remote` both refuse to run when that shell binds anything that would redirect a shipped binary off the QA profile (`SPACESD_EXECUTABLE`, `SPACES_RUNTIME_DIR`, `SPACES_CADDY_EXECUTABLE`, the `SPACES_DEVICE_API_*` trio, `SPACES_DB_PATH`, `SPACES_CLIENT_DB_PATH`, `SPACES_CLIENT_SECRET_DIR`, `SPACES_GHOSTTY_RESOURCES_DIR`); they name what is bound instead of clearing it.

Some of what the QA app does is shipped behavior rather than a lane artifact. Its launch repair re-asserts the installed `~/.spaces/bin` symlinks and the `dev.usespaces.spacesd` LaunchAgent plist, which is idempotent here because they already name the same installed bundle; Sparkle runs as usual, so an update prompt is the real one. Desktop control (the global hotkey, window cycling) is a machine-wide lease held by one process at a time, so it reaches the QA app only while the installed app is not running.

`stop` stops the app named in the QA profile's own app-owner lease and asks the daemon answering the QA profile's own socket to shut down. Both are addressed per profile and never by process name, because the QA daemon is the same `spacesd` binary as the installed one. `remove` deletes `~/.spaces-dev/qa` once neither is running, reading the profile's app-owner lease and its daemon instance lock rather than an unanswered ping, since the daemon unlinks its socket when it binds rather than when it exits.

`deploy-remote` covers the Linux daemon. It reads the installed app's version from its `Info.plist`, downloads the released `spacesd-ubuntu-24.04-<arch>` artifact from the matching `v<version>` GitHub release, verifies its published sha256, and runs the bundled `install.sh --profile qa` on the device, which lays that profile under `~/.spaces-dev/profiles/spaces/qa/` and runs it as `spacesd@qa.service`. It then opens a pairing window through that profile's own CLI and redeems the link with the installed `spaces` CLI bound to the QA database, so both ends of the pairing are shipped code. The environment check runs before the first remote command, so a shell that would redirect the installed CLI refuses the deploy rather than failing at the pairing step with a release already installed. The device comes from the same `.env` remote keys every other remote workflow uses, so load that env first; tear the remote profile down with `spacese2e profile remove --remote qa`.
...
In CI the same release is cut by pushing a version tag, which runs [`.github/workflows/release.yml`](../.github/workflows/release.yml), a thin caller of the reusable [`release-build.yml`](../.github/workflows/release-build.yml). Every tagged release is published as a GitHub prerelease and reaches the stable update feed only when it is promoted; see [Release Promotion](#release-promotion).
...
Sparkle update hosting lives under `https://usespaces.dev/releases/` on the static Firebase site, which Next.js exports as real static files before Firebase deploy. A Firebase Hosting deploy replaces the whole site, so GitHub release state is the source of truth for what that directory contains: `scripts/stage-web-releases.sh` runs from the `apps/web` `prebuild` and stages two feeds on every build. `releases/appcast.xml` is the stable feed, taken from `releases/latest/download`, which GitHub resolves to the newest release that is not a prerelease. `releases/prerelease/appcast.xml` is the pre-release feed, taken from the newest `v<major>.<minor>.<patch>` release whether or not it is still flagged prerelease. Those two appcasts are the only files the site carries: `/releases/Spaces-<version>.zip` is a 302 redirect to `https://github.com/yogesh-dhande/spaces/releases/download/v<version>/Spaces-<version>.zip`, configured as a regex `redirects` rule in [`apps/web/firebase.json`](../apps/web/firebase.json). That rule captures the archive name alongside the version and rebuilds the filename from both, because the redirect engine substitutes captures only when the destination names one directly after a path separator, and a destination that referenced the version alone would be emitted with `:version` unexpanded. Hosting stores every deployed version of the site, and the two Sparkle archives are about 116 MB each, so carrying them on the site exhausts the storage quota and blocks deploys outright. The enclosure URLs keep naming usespaces.dev because `publish-sparkle-appcast.sh` bakes `https://usespaces.dev/releases` in as the enclosure prefix at EdDSA signing time and no appcast is ever rewritten or re-signed afterwards, which is what keeps promotion a pure GitHub flag flip; the redirect is what resolves that URL. Sparkle fetches the enclosure through `NSURLSession`, which follows the redirect, and verifies the signature over the bytes it downloaded. Each staged appcast is checked against both halves of that arrangement: its enclosure URL must be exactly the `https://usespaces.dev/releases/Spaces-<version>.zip` form the redirect rule matches, and the GitHub asset the redirect targets must exist, so a feed can never advertise a download that 404s. Nothing rewrites or re-signs an appcast, at staging or at promotion. Any website build (merge deploy, PR preview, release, promotion, local) republishes both feeds complete instead of blanking them, and a release that failed to publish its appcast and zip fails the next site build loudly. The script lists releases with `gh`, so a local `npm run build` in `apps/web` needs an authenticated `gh` (`gh auth login`) and CI web builds pass `GH_TOKEN`. The Linux installer is published the same way, copied to `https://usespaces.dev/install.sh` by the same `prebuild` rather than as a GitHub release asset. The app bundle carries `spaces`, `spacesd`, and Caddy in `Contents/Resources`; the DMG installer links `/usr/local/bin` and `~/.spaces/bin` helpers to those bundled binaries so installed CLI commands, launchd, and remote Mac pairing use the updated app bundle after Sparkle updates. Linux artifacts link `~/.local/bin/spaces` to the managed `~/.spaces/bin/spaces` helper so user shells can run `spaces` without a system-wide install.

## Release Promotion

A tagged release starts as a GitHub prerelease and is promoted by hand once it has been tested. There is one binary, one appcast, and one set of artifacts per release; promotion changes only which feed serves it.

1. Push a version tag. `release.yml` builds and creates the GitHub release with `--prerelease`, pinned with `--target` to the tagged commit, and never passes `--latest`. It then deploys the website from the tagged commit and confirms `https://usespaces.dev/releases/prerelease/appcast.xml` serves the new version, so Macs with "Receive pre-release updates" on can take it immediately.
2. Test the prerelease.
3. Promote it: edit the release and select **Latest**. The releases UI offers pre-release and latest as one radio choice, so picking Latest clears the pre-release flag and pins `releases/latest` in a single update. The CLI equivalent is `gh release edit <tag> --prerelease=false --latest`. Pinning `latest` is the half that matters to installed Macs: it is what the stable feed and `install.sh` resolve through. The `released` event then runs [`release-promote.yml`](../.github/workflows/release-promote.yml), which checks out the release's commit, confirms `releases/latest` resolves to the promoted tag before building anything, redeploys the website, and confirms `https://usespaces.dev/releases/appcast.xml` serves the promoted version. Releases created by the Actions token do not raise `released`, so the tag build cannot trigger this workflow. Promotion deploys the released commit's site but runs the deploy and feed-verification scripts from `main`, checked out into `.promote-tooling`, so a release tagged before a change to those scripts still promotes.

A prerelease that fails testing is simply left as a prerelease; the fix ships under the next patch tag. Because the pre-release feed always names the newest tagged release, a Mac that already took the bad build moves to the fix on its next update check without any manual intervention.

`install.sh`'s no-version path resolves `releases/latest/download`, so it installs the newest promoted release; its version-pinned form installs any release, promoted or not.

Promotion never touches iOS. TestFlight is the iOS pre-release tier, and App Store submission is manual.

## iOS Release

[`.github/workflows/ios-release.yml`](../.github/workflows/ios-release.yml) builds the iOS app and uploads it to App Store Connect for TestFlight. Pushing a version tag runs both the macOS and iOS release workflows, so the same version ships across every client. A `workflow_dispatch` run with a required `version` input cuts a TestFlight-only build from any branch without tagging a public macOS release. The build number is `GITHUB_RUN_NUMBER` for both triggers. `scripts/sync-app-version.sh --short <version> --build <build>` stamps the shared version metadata while preserving the checked-in Sparkle feed URL and keys.

The workflow obtains `GhosttyKit` the same way as the macOS release, running `apps/macos/scripts/ensure_ghostty_artifacts.sh --publish-missing` so the pinned-submodule `GhosttyKit.xcframework` (including its iOS device and simulator slices) is present, then `xcodebuild archive`/`-exportArchive` with [`apps/ios/ExportOptions.plist`](../apps/ios/ExportOptions.plist) (`method: app-store-connect`, `destination: upload`). The export step itself uploads the build to App Store Connect. The `.xcarchive` is retained as a workflow artifact for debugging. In `apps/ios/project.yml`, the `SpacesMobile` scheme's build targets list `SpacesMobileTests`, `SpacesMobileSmokeUITests`, and `SpacesMobileUITests` with `[test]` rather than `all`, so their `BuildActionEntry` gets `buildForArchiving="NO"`: those targets `@testable import SpacesMobile`, which needs the app built with `-enable-testing`, a flag `xcodebuild archive -configuration Release` does not set. Leaving the test targets on `all` would have `archive` compile them in Release and fail every release build; XcodeGen regenerates the shared scheme from this setting, so hand-editing the generated `.xcscheme` does not stick.
...
Firebase Hosting deploys from [`.github/workflows/firebase-hosting-merge.yml`](../.github/workflows/firebase-hosting-merge.yml). It builds `apps/web` and deploys the static export on pushes to `main` that touch the site or the release staging script; [`firebase-hosting-pull-request.yml`](../.github/workflows/firebase-hosting-pull-request.yml) builds the same site and posts a preview channel on pull requests. The release and promotion workflows deploy the same way, so every deploy republishes both Sparkle feeds from GitHub release state.

The three live-channel deploys (merge, release, promotion) run [`scripts/deploy-firebase-hosting.sh`](../scripts/deploy-firebase-hosting.sh), which deploys `apps/web` with a pinned `firebase-tools` against project `spaces-a1814` and reads the CLI's `--json` result. It fails the step on a deploy error, and treats exactly one error as success: HTTP 400 "is the current active version", which means the built site is byte-identical to the live one and the deploy is a no-op. The action fails its step on a deploy error, but tolerating the no-op with `continue-on-error` on the release and promotion deploys also hid every real error, such as a Hosting storage quota HTTP 429 reaching the job as a green deploy step; the pull request workflow stays on the action because it needs the preview channel and its pull request comment, and it fails on a deploy error there because it sets no `continue-on-error`. The script reads the service account from `FIREBASE_SERVICE_ACCOUNT`, set from the `FIREBASE_SERVICE_ACCOUNT_SPACES_A1814` secret, and hands it to the CLI as a temporary credentials file it deletes on exit. Because the site's `prebuild` reads GitHub releases with `gh`, every workflow that builds `apps/web` passes `GH_TOKEN`.

The Hosting site keeps a release storage limit of five versions, set in the Firebase console under Hosting, Release history, so older versions are deleted automatically instead of accumulating against the storage quota.

[`apps/web/firebase.json`](../apps/web/firebase.json) is the only Hosting config: it sets the public directory, headers, the release redirect, and the app rewrite, and every deploy, preview and live, runs against `apps/web`.
