...
A nightly version is the newest stable tag plus a UTC date component, `0.5.1.20260725`, and its `CFBundleVersion` is `YYYYMMDDHHMM`. Four dotted numeric components are required rather than a `-nightly` suffix: `SpacesWireProtocol.isVersion(_:olderThan:)` maps non-numeric components to `0`, so a suffixed version would sort below the release it was built from and suppress the staged-update hint. The minute-precision build number is what Sparkle compares, so a second dispatch on the same day is still offered as an update. A scheduled run whose `main` tip already has a nightly tag exits before building; a manual dispatch always builds.
