...
- `spaces terminal send`, `key`, and `takeover` are same-profile local automation commands. They operate on the local per-session Unix control socket and are not accepted through remote daemon or mobile control paths without authenticated Device API owner checks.
...
- Device API settings live under the terminal root in `device-api.json`; the active Device API port and profile transport key persist there, and the transport key rotates when all pairings are reset.
- `spaces pair` asks the same-machine daemon to open a five-minute pairing window and prints the `spaces://` pairing link, code, expiry, and endpoint fingerprint. `spaces pair --json` prints structured pairing metadata for SSH-assisted remote-device pairing. Remote Macs must be installed from the DMG so `~/.spaces/bin/spaces` points at the supported CLI. Supported Linux devices are prepared automatically by the Mac app over SSH when pairing needs it. `spacese2e mobile-status`, `mobile-serve`, and `mobile-request` remain harness-only commands for status and transport tests.
- The Mac sidebar Devices action opens a compact panel with the connected daemon list, active-device selection, a five-minute QR pairing window for iPhone and iPad clients under each connected daemon, and an SSH target form for pairing this Mac with a remote `spacesd`.
...
- The Device API serves workspace and terminal overview data plus authenticated attach, subscribe, takeover, send, key, workspace-creation, workspace-terminal, process, and coding-agent lifecycle requests over the same session boundary.
- macOS daemons also serve the authenticated `launchSpacesApp` recovery request. It checks the current profile app-owner lease before spawning, returns success immediately when `SpacesApp` already owns the profile, and otherwise starts the resolved `SpacesApp` executable with `SPACES_DB_PATH`, `SPACES_RUNTIME_DIR`, and `SPACESD_EXECUTABLE` set for the current profile and service executable.
...
- iOS owner rendering uses `GhosttyRemoteTerminalView`. It forwards the same precise deltas and momentum metadata through display-frame coalescing, but it also scrolls the cached render-frame snapshot locally for immediate touch feedback while the Device API request is in flight. The local snapshot viewport is row-based, so pixel deltas are divided by cell height and sub-row remainders accumulate until they become a whole row. Tiny swipes therefore stay tiny instead of forcing a one-row jump, and repeated tiny swipes still add up to visible scrollback movement.
