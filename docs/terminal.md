...
- Mac terminal windows use Mac `spacesd` as the profile endpoint. The Mac app keeps a local SQLite mirror of remote launch, runtime, attachment, and final render-state metadata, while live attach, takeover, input, resize, scroll, and state refresh requests are proxied to the workspace's owning daemon. iOS terminal detail uses direct pinned-TLS access to the owning remote daemon with its scoped terminal credential, and uses the Mac bridge for local sessions.
...
- `spaces terminal send`, `key`, and `takeover` are same-profile local automation commands. They operate on the local per-session Unix control socket and are not accepted through remote daemon or mobile control paths without the authenticated bridge and owner checks.
- `spacesd` publishes the first-party TLS-PSK bridge consumed by the iOS client. The Mac app owns user-facing bridge pairing and status, while `spacese2e` provides harness-only status, request, and standalone bridge commands.
...
- `SpacesMobile` discovers the Mac bridge through Bonjour or accepts manual host entry, reads workspace metadata from Mac `spacesd`, and routes local terminal state, control, link-preview, and chunk reads through that Mac bridge. Remote terminal rows carry a provisioned scoped credential; iOS stores it in Keychain keyed by the pinned daemon endpoint and sends terminal-safe requests directly to the remote daemon. The iOS client does not receive remote compute-host admin tokens. It keeps one selected terminal detail at a time, auto-attempts takeover for live opened sessions, renders the owner path from service-published Ghostty render frames, and renders ended sessions from persisted final Ghostty state.
...
- `spacese2e mobile-status` shows the active port, Bonjour service name, endpoint fingerprint, and reachable IPv4 addresses for harnesses. `spacese2e mobile-serve --host ... --port ... --pairing-code ...` runs the bridge as a standalone process, opens a five-minute pairing window, and prints the full pairing link, code, expiry, host, port, and endpoint fingerprint. Harnesses that need to issue bridge JSON use `spacese2e mobile-request` with the pairing link or transport key so they exercise the same TLS-PSK transport as the iOS app. Standalone bridge processes do not service `launchSpacesApp` because they do not own the spacesd executable context.
...
- Mobile overview payloads may include remote daemon host, port, certificate fingerprint, and a scoped terminal-only mobile token. They omit the compute-host admin token. The Mac bridge resolves and applies the admin token only for Mac-side remote provisioning and session discovery.
