...
- `spaces pair` asks the same-machine daemon to open a five-minute pairing window and prints the `spaces://` pairing link, code, expiry, and endpoint fingerprint. `spaces pair --json` prints structured pairing metadata for SSH-assisted Mac remote-device pairing. `spacese2e mobile-status`, `mobile-serve`, and `mobile-request` remain harness-only commands for status and transport tests.
