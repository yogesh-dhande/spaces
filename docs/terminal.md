...
- Image paste is an image-only extension of the same owner input boundary. The macOS window controller reads PNG/JPEG/TIFF pasteboard data and image file URLs from the local pasteboard, normalizes TIFF data to PNG, rejects images over 10 MiB before upload, and leaves non-image `Ctrl+V` as ordinary terminal input. Accepted images are sent to the owning daemon with the current client ID and owner epoch; the daemon writes a `/tmp/spaces-paste-<uuid>.<ext>` file and injects only that path string into the PTY.
