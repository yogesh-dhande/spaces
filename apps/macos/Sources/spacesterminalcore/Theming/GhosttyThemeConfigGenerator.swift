...
public enum GhosttyThemeConfigGenerator {
    ...
    /// loop; `font-size = 12` is the baseline every embedded terminal starts from, matching
    /// `TerminalTextSize.default`. A client surface carries its own resolved size into surface
    /// creation, so this value governs only a surface built before any size is applied.
    /// `clipboard-write-limit-bytes = 1048576` matches
    /// `GhosttyEmbeddedClipboardWriteForwarder.maximumClipboardWriteBytes` (1 MiB) and
    /// `SPACES_GHOSTTY_VT_MAX_CLIPBOARD_BYTES` in the Linux vt shim, so Ghostty refuses an
    /// oversized Kitty clipboard (OSC 5522) write with EFBIG before spooling it instead of
    /// buffering up to its 64 MiB default and having Spaces drop it afterwards.
    static func rootConfigContents(lightThemePath: String, darkThemePath: String) -> String {
        ...
        window-vsync = false
        font-size = 12
        clipboard-write-limit-bytes = 1048576
        """ + "\n"
    }
