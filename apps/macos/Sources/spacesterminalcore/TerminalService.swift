...
public enum TerminalService {
    ...
    public static func createSession(_ launchConfiguration: TerminalSessionLaunchConfiguration) throws -> TerminalServiceSessionSummary {
        ...
        guard response.ok else { throw TerminalServiceError.requestFailed(response.message) }
        guard let session = response.session else { throw TerminalServiceError.requestFailed("Terminal service did not return a session summary.") }
        ...
        -> TerminalServiceSessionSummary
    {
        ...
        let runtimeState =
            ...
            ?? TerminalSessionRuntimeState(
                sessionID: launchConfiguration.sessionID, backend: launchConfiguration.backend, servicePID: getpid(), childPID: nil, state: .running,
                updatedAt: ISO8601DateFormatter().string(from: Date()), title: launchConfiguration.title,
    ...
    private static func terminateXCTestCompatibilitySession(id sessionID: String) throws {
        ...
        let exitedState = TerminalSessionRuntimeState(
            sessionID: sessionID, backend: runtimeState.backend, servicePID: runtimeState.servicePID, childPID: runtimeState.childPID, state: .exited,
            updatedAt: now, exitedAt: now, title: runtimeState.title ?? launchConfiguration.title,
    ...
    private static func resolveExecutableURL(environment: [String: String] = ProcessInfo.processInfo.environment, fileManager: FileManager = .default)
        throws -> URL
    {
