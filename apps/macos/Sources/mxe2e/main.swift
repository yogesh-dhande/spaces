...
private struct SelectWorkspaceDetailCommand: ParsableCommand {
    ...
    /// Tells the running Muxy app to show one workspace detail pane by
...
private struct CleanupFixturesCommand: ParsableCommand {
    ...
    /// `repo` entries in the current Muxy database.
    func run() throws {
        let orchestrator = try makeOrchestrator()
...
private struct SeedFixtureCommand: ParsableCommand {
    ...
    /// Registers a local git repo as a Muxy project and seeds deterministic
...
private func workspaceSummary(orchestrator: MuxyOrchestrator, projectDir: String, title: String) throws -> WorkspaceSummaryPayload? {
    let normalizedProjectDir = normalizePath(projectDir)
...
private func makeOrchestrator() throws -> MuxyOrchestrator { try MuxyOrchestrator(store: .init(path: DatabaseLocator.defaultPath())) }
