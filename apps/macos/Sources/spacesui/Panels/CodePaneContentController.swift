...
@MainActor final class CodePaneContentController: NSObject, PaneContentHosting {
    ...
    /// completes synchronously, so a hibernated or just-installed pane costs quitting nothing. A
    /// close, retarget, or deactivate that tears the page down before either arrives also settles
    /// the flush immediately, in `teardownWebView()`, since a torn-down page can never answer.
    ///
    ...
    private func teardownWebView() {
        ...
        if let pending = pendingEditsFlush { completeEditsFlush(token: pending.token) }
