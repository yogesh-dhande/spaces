...
@MainActor public final class AppUpdater {
    ...
    func downloadAndInstall(from url: URL) async throws {
        let tempDir = FileManager.default.temporaryDirectory.appending(path: "spaces-update-\(UUID().uuidString)")
        ...
        let targetApp = applicationsDir.appending(path: "Spaces.app")
    ...
    private func findAppBundle(in directory: URL) throws -> URL {
        ...
        for item in contents { if item.pathExtension == "app" && item.lastPathComponent == "Spaces.app" { return item } }
