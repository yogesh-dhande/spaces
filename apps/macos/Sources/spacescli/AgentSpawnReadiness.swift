...
/// a submit so its Enter lands as a distinct keystroke. Spawn returns only when both hold, which is what
/// makes the orchestrator's first `terminal send --submit` safe to issue immediately.
///
...
enum AgentSpawnReadiness {
    ...
    /// Polls `snapshot` until the child is a detected coding agent that is ready for input, the child's
    ...
    static func awaitReadiness(
        ...
        while true {
            ...
            if let kind = current.detectedKind, current.bracketedPasteActive { return .ready(kind) }
