...
public struct HotkeySpec: Sendable, Equatable {
    ...
    public var keyCode: UInt32 {
        #if canImport(Carbon)
            if let code = HotkeySpec.keyCodeMap[key] { return code }
    ...
    public static func parse(_ raw: String) throws -> HotkeySpec {
        ...
        guard !trimmed.isEmpty else { throw HotkeySpecError("Hotkey cannot be empty") }
        ...
        guard !tokens.isEmpty else { throw HotkeySpecError("Hotkey cannot be empty") }
        ...
        for tokenSub in tokens {
            ...
            if keyToken != nil { throw HotkeySpecError("Hotkey has multiple keys: \(keyToken ?? "") and \(token)") }
            if let canonicalKey = canonicalKeyName(token) { keyToken = canonicalKey } else { throw HotkeySpecError("Unsupported key: \(token)") }
        }
    ...
    private static func canonicalKeyName(_ token: String) -> String? {
        if token.count == 1, let char = token.unicodeScalars.first {
            if CharacterSet.letters.contains(char) { return token }
            if CharacterSet.decimalDigits.contains(char) { return token }
            let allowedPunctuation: Set<String> = ["[", "]", ";", "'", ",", ".", "/", "\\", "=", "`"]
            if allowedPunctuation.contains(token) { return token }
        }
        ...
        case "leftbracket", "lbracket": return "["
        case "rightbracket", "rbracket": return "]"
        ...
        case "forwarddelete", "forward-delete": return "forwarddelete"
        case "left", "right", "up", "down": return token
        default: break
        }

        if token.hasPrefix("f"), token.count >= 2 {
            let suffix = token.dropFirst()
            if let value = Int(suffix), value >= 1, value <= 20 { return "f\(value)" }
        }
        ...
        private static let keyCodeMap: [String: UInt32] = [
            ...
            "z": UInt32(kVK_ANSI_Z), "0": UInt32(kVK_ANSI_0), "1": UInt32(kVK_ANSI_1), "2": UInt32(kVK_ANSI_2), "3": UInt32(kVK_ANSI_3),
            "4": UInt32(kVK_ANSI_4), "5": UInt32(kVK_ANSI_5), "6": UInt32(kVK_ANSI_6), "7": UInt32(kVK_ANSI_7), "8": UInt32(kVK_ANSI_8),
            "9": UInt32(kVK_ANSI_9), "=": UInt32(kVK_ANSI_Equal), "minus": UInt32(kVK_ANSI_Minus), "[": UInt32(kVK_ANSI_LeftBracket),
            "]": UInt32(kVK_ANSI_RightBracket), ";": UInt32(kVK_ANSI_Semicolon), "'": UInt32(kVK_ANSI_Quote), ",": UInt32(kVK_ANSI_Comma),
            ".": UInt32(kVK_ANSI_Period), "/": UInt32(kVK_ANSI_Slash), "\\": UInt32(kVK_ANSI_Backslash), "`": UInt32(kVK_ANSI_Grave),
            "space": UInt32(kVK_Space), "tab": UInt32(kVK_Tab), "return": UInt32(kVK_Return), "enter": UInt32(kVK_Return),
            "escape": UInt32(kVK_Escape), "delete": UInt32(kVK_Delete), "backspace": UInt32(kVK_Delete), "forwarddelete": UInt32(kVK_ForwardDelete),
            "left": UInt32(kVK_LeftArrow), "right": UInt32(kVK_RightArrow), "up": UInt32(kVK_UpArrow), "down": UInt32(kVK_DownArrow),
        ]
    #endif
}
