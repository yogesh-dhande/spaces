...
public struct ProjectRecord: Codable, Sendable {
    ...
    /// Whether this is an ordinary project or the daemon's own home project. Set when the record is
    /// created and never edited afterwards, so it is a `let` that neither project settings nor the
    /// `spaces.yaml` round trip can move.
    ...
    public init(
        id: String, name: String, dir: String, isGitRepo: Bool, defaultBranch: String?, kind: ProjectKind, isHidden: Bool = false,
        setupScript: String? = nil, stopScript: String? = nil, ports: [ServiceDefinition] = [], processes: [ProcessTemplate] = [],
