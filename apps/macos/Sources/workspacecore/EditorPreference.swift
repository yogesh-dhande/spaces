...
/// A GUI code editor the client can launch for a workspace.
///
/// The editor is located by bundle identifier (rename-proof) and driven through its
/// command-line tool. Remote opens present the workspace as a local window backed by the
/// remote filesystem, and re-launching the same folder focuses the editor's existing
/// window — the client's editor focus mechanism (see `docs/implementation.md`). The
/// VS Code family and Zed differ in how the remote workspace is addressed; see `family`.
public enum EditorPreference: String, Codable, Sendable, CaseIterable {
    ...
    /// macOS bundle identifier used to launch and detect the editor; `nil` for `.none`.
    ///
    ...
    public var family: EditorFamily {
        switch self {
        // `.none` is filtered out before launch; its value here is never consulted.
        case .none, .vscode, .devin: return .vscode
        case .zed: return .zed
        }
    }
    ...
    public var installableRemoteSSHExtensionID: String? {
        ...
        case .devin, .zed, .none: return nil
        }
    }
}
