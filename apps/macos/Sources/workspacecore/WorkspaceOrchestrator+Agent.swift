...
extension WorkspaceOrchestrator {
    ...
    func trackedTerminalWindowsForAgents(workspaceID: String, agentWindows: [AgentWindowRecord]) throws -> [WindowRecord] {
        ...
        // Coding agents register their own tracked terminal rows independently of a workspace launch.
        // Preserve those rows when replacing the workspace window snapshot so a stop can still close the
        // actual agent terminals.
    ...
    func recordAgentExitStatus(_ existing: AgentWindowRecord) throws -> AgentWindowRecord? {
        let now = nowISO8601()
        guard try store.markAgentWindowExitStatus(existing, status: .exited, updatedAt: now) else { return nil }
