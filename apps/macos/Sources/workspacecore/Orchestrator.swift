...
public final class WorkspaceOrchestrator {
    ...
    public func updateWorkspaceName(workspaceID: String, name: String) throws {
        ...
        if let existing = try store.workspace(projectID: workspace.projectID, name: trimmedName), existing.id != workspace.id {
            throw WorkspaceError.workspaceAlreadyExists(project: project.name, workspace: trimmedName)
        }
        ...
        if let title {
            ...
            if trimmedTitle != workspace.title {
                if let existing = try store.workspace(projectID: workspace.projectID, name: trimmedTitle), existing.id != workspace.id {
                    throw WorkspaceError.workspaceAlreadyExists(project: project.name, workspace: trimmedTitle)
                }
    ...
    public func createWorkspace(
        ...
        if let existing = try store.workspace(projectID: projectID, name: trimmedName) {
            if !existing.isArchived { throw WorkspaceError.workspaceAlreadyExists(project: project.name, workspace: trimmedName) }
    ...
    public func createWorkspaceFromWorktree(worktreePath: String, name: String? = nil) throws -> WorkspaceRecord {
        ...
        if let existing = try store.workspace(projectID: projectID, name: inferredName), !existing.isArchived {
            throw WorkspaceError.workspaceAlreadyExists(project: project.name, workspace: inferredName)
        }
    ...
    private func restartProcessInTerminal(
        ...
        let restartedProcess = RunningProcessRecord(
            ...
            pid: newPID, status: .running, logPath: nil, lastOutputAt: nil, startedAt: nowISO8601(), exitedAt: nil)
        ...
        for process in try store.runningProcesses(workspaceID: workspaceID) where isManagedTerminalApp(process.terminalApp) {
            try store.upsert(
                runningProcess: RunningProcessRecord(
                    ...
                    terminalNativeID: process.terminalNativeID, terminalContainerID: process.terminalContainerID, itermTabIndex: nil,
            ...
            guard let liveTmuxWindow = liveWindowsByID[tmuxWindowID] else {
                ...
                try store.upsert(
                    runningProcess: RunningProcessRecord(
                        ...
                        terminalNativeID: process.terminalNativeID, terminalContainerID: process.terminalContainerID,
                        itermTabIndex: process.itermTabIndex, tmuxWindowID: process.tmuxWindowID, pid: process.pid, status: .exited,
                        logPath: process.logPath, lastOutputAt: process.lastOutputAt, startedAt: process.startedAt, exitedAt: process.exitedAt ?? now)
                )
                didMutate = true
                continue
            }
            ...
            if process.status == .exited, let panePID = liveTmuxWindow.panePID, panePID > 0, paneAlive {
                try store.upsert(
                    runningProcess: RunningProcessRecord(
                        ...
                        terminalApp: process.terminalApp, windowID: process.windowID, terminalTrackingID: process.terminalTrackingID,
                        terminalNativeID: process.terminalNativeID, terminalContainerID: process.terminalContainerID,
                        itermTabIndex: process.itermTabIndex, tmuxWindowID: process.tmuxWindowID, pid: panePID, status: .running,
                        logPath: process.logPath, lastOutputAt: process.lastOutputAt, startedAt: process.startedAt, exitedAt: nil))
                didMutate = true
            }
        }
    ...
    private func recoverRunningProcessTerminalIfPossible(workspaceID: String, process: RunningProcessRecord) throws -> Bool {
        ...
        try store.upsert(
            runningProcess: RunningProcessRecord(
                ...
                terminalContainerID: terminalContainerID, itermTabIndex: nil, tmuxWindowID: process.tmuxWindowID, pid: pid, status: .running,
                logPath: process.logPath, lastOutputAt: process.lastOutputAt, startedAt: process.startedAt, exitedAt: nil))
