...
enum DatabaseSchema {
    ...
    static let migrationSteps: [DatabaseMigrationStep] = [
        ...
        DatabaseMigrationStep(
            fromVersion: 2, toVersion: 3, description: "Add canonical runtime target links for processes and agent sessions", requiresBackup: true,
            apply: { db in try migrateV2toV3(db: db) }),
        DatabaseMigrationStep(
            fromVersion: 3, toVersion: 4, description: "Generalize agent session event target linkage", requiresBackup: true,
            apply: { db in try migrateV3toV4(db: db) }),
    ]
    ...
    private static func migrateV2toV3(db: OpaquePointer) throws {
        try executeMigrationSQL(db: db, sql: "ALTER TABLE running_processes ADD COLUMN runtime_target_id TEXT;")
        try executeMigrationSQL(db: db, sql: "ALTER TABLE agent_sessions ADD COLUMN runtime_target_id TEXT;")

        try executeMigrationSQL(
            db: db,
            sql: """
                UPDATE agent_sessions
                SET runtime_target_id = COALESCE(runtime_target_id, terminal_target_id)
                WHERE runtime_target_id IS NULL OR runtime_target_id = ''
                """)
        ...
        try executeMigrationSQL(
            ...
            sql: """
                ...
                SET runtime_target_id = (
                  ...
                  WHERE rt.workspace_id = running_processes.workspace_id
                    AND (
                      (running_processes.terminal_tracking_id IS NOT NULL AND running_processes.terminal_tracking_id != '' AND tt.tracking_id = running_processes.terminal_tracking_id)
                      OR (running_processes.terminal_native_id IS NOT NULL AND running_processes.terminal_native_id != '' AND tt.native_id = running_processes.terminal_native_id)
                      OR (running_processes.tmux_window_id IS NOT NULL AND running_processes.tmux_window_id != '' AND tt.tmux_window_id = running_processes.tmux_window_id)
                      OR (running_processes.window_id IS NOT NULL AND rt.window_id = running_processes.window_id)
                    )
    ...
    private static func migrateV3toV4(db: OpaquePointer) throws {
        try executeMigrationSQL(db: db, sql: "ALTER TABLE agent_session_events ADD COLUMN runtime_target_id TEXT;")
        try executeMigrationSQL(
            db: db,
            sql: """
                UPDATE agent_session_events
                SET runtime_target_id = terminal_target_id
                WHERE (runtime_target_id IS NULL OR runtime_target_id = '')
                  AND terminal_target_id IS NOT NULL
