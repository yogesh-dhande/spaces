...
enum DatabaseSchema {
    ...
    private static func migrateV6ToV7(db: OpaquePointer) throws {
        try execute(db: db, sql: "ALTER TABLE workspaces RENAME TO workspaces_v6;")
        try execute(
            db: db,
            sql: """
                CREATE TABLE workspaces (
        ...
        try rebuildTable(
            ...
            copySQL: """
                INSERT INTO workspace_ports(workspace_id, port_index, port_number, port_name, definition_id)
                SELECT workspace_id, port_index, port_number, port_name, definition_id FROM workspace_ports_v6
        ...
        try rebuildTable(
            ...
            copySQL: """
                INSERT INTO workspace_port_definitions(id, workspace_id, name, order_index)
                SELECT id, workspace_id, name, order_index FROM workspace_port_definitions_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "workspace_settings",
            createSQL: """
            ...
            copySQL: """
                INSERT INTO workspace_settings(workspace_id, stop_script, setup_status, setup_error, setup_started_at, setup_finished_at)
                SELECT workspace_id, stop_script, setup_status, setup_error, setup_started_at, setup_finished_at FROM workspace_settings_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "workspace_processes",
            createSQL: """
            ...
            copySQL: """
                ...
                SELECT id, workspace_id, name, command, on_exit, execution_mode, order_index FROM workspace_processes_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "workspace_status_checks",
            createSQL: """
            ...
            copySQL: """
                INSERT INTO workspace_status_checks(id, workspace_id, name, process, command, interval, timeout, on_fail, order_index)
                SELECT id, workspace_id, name, process, command, interval, timeout, on_fail, order_index FROM workspace_status_checks_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "workspace_browser_sessions",
            createSQL: """
            ...
            copySQL: """
                INSERT INTO workspace_browser_sessions(id, workspace_id, name, url, extracted_target_url, extracted_window_id, extracted_window_valid, order_index)
                SELECT id, workspace_id, name, url, extracted_target_url, extracted_window_id, extracted_window_valid, order_index
                FROM workspace_browser_sessions_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "workspace_agent_launchers",
            createSQL: """
            ...
            copySQL: """
                INSERT INTO workspace_agent_launchers(id, workspace_id, name, command, order_index)
                SELECT id, workspace_id, name, command, order_index FROM workspace_agent_launchers_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "running_processes",
            createSQL: """
        ...
        try rebuildTable(
            ...
            copySQL: """
                INSERT INTO status_results(process_id, check_name, status, message, last_run_at)
                SELECT process_id, check_name, status, message, last_run_at FROM status_results_v6
                """, previousVersionSuffix: "v6")
        try rebuildTable(
            db: db, tableName: "windows",
            createSQL: """
        ...
        try execute(
            db: db,
            sql: """
                CREATE UNIQUE INDEX workspaces_project_branch_unique
                ON workspaces(project_id, branch)
                WHERE length(branch) > 0
                """)
    }
