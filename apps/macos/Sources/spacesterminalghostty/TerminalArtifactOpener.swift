    ...
    public enum TerminalArtifactCategory: Sendable, Equatable, Hashable {
        ...
        public static func category(forFileURL url: URL) -> TerminalArtifactCategory? {
            guard
                let kind = SpacesDeviceTerminalLinkClassifier.artifactKind(
                    contentType: nil, pathExtension: url.pathExtension)
            else { return nil }
    ...
    @MainActor
    public struct TerminalArtifactHandlerRegistry {
        ...
        public init(
            handlers: [TerminalArtifactCategory: Handler],
            defaultOpenHandler: @escaping Handler = { NSWorkspace.shared.open($0) }
        ) {
        ...
        public static func defaultRegistry() -> TerminalArtifactHandlerRegistry {
            ...
            return TerminalArtifactHandlerRegistry(handlers: [
                .webURL: openInBrowser,
                .html: openInBrowser,
                .image: openInWorkspace,
                .video: openInWorkspace,
                .pdf: openInWorkspace,
                .markdown: openInWorkspace,
                .text: openInWorkspace,
            ])
        }

        @discardableResult
        public func open(_ url: URL, as category: TerminalArtifactCategory) -> Bool {
        ...
        @discardableResult
        public func openLocalFile(at url: URL) -> Bool {
            guard let category = TerminalArtifactCategory.category(forFileURL: url) else {
                return defaultOpenHandler(url)
            }
