...
struct DatabaseMigrator {
    ...
    func migrateIfNeeded(
        ...
        guard var version = schemaVersion else {
            throw MuxyError.databaseMigrationFailed(message: "Unsupported database schema at \(databasePath): missing migration_state marker.")
        }
        guard version <= currentSchemaVersion else {
            throw MuxyError.databaseMigrationFailed(message: "Unsupported database schema version \(version) at \(databasePath).")
        }
        ...
        while version < currentSchemaVersion {
            guard let step = steps.first(where: { $0.fromVersion == version }) else {
                throw MuxyError.databaseMigrationFailed(
        ...
        do { try validateIntegrity() } catch {
            ...
            throw MuxyError.databaseMigrationFailed(message: "Database migration integrity check failed for \(databasePath).\(backupMessage)")
        }
    }

    private func migrationError(for step: DatabaseMigrationStep, underlying: Error, backupURL: URL?) -> MuxyError {
        let backupMessage = backupURL.map { " A pre-migration backup was saved to \($0.path)." } ?? ""
