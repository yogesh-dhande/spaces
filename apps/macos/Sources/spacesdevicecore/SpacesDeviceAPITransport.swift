...
public enum SpacesDeviceAPITransportRole: Sendable {
    case server
    case client
}

public enum SpacesDeviceAPITransportError: LocalizedError, Equatable {
    case missingTransportKey
    case invalidTransportKey

    public var errorDescription: String? {
        switch self {
        case .missingTransportKey: "The Device API transport key is missing."
        case .invalidTransportKey: "The Device API transport key is invalid."
        }
    }
}

public enum SpacesDeviceAPITransport {
    public static let pskIdentity = "spaces-device-api-v1"

    #if canImport(Network) && canImport(Security)
        public static func parameters(transportKey: String, role: SpacesDeviceAPITransportRole) throws -> NWParameters {
    ...
    public static func decodeTransportKey(_ value: String) throws -> Data {
        ...
        guard !trimmed.isEmpty else { throw SpacesDeviceAPITransportError.missingTransportKey }
        ...
        guard let data = Data(base64Encoded: base64), data.count >= 32 else { throw SpacesDeviceAPITransportError.invalidTransportKey }
        return data
    }
