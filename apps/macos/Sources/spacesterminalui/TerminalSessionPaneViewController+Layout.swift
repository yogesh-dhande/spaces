...
extension TerminalSessionPaneViewController {
    func buildUI() {
        ...
        // Queryable regardless of the pane's window shell: UI automation and VoiceOver read the
        ...
        // which does not distinguish attachment mode since panes share one window.
    ...
    /// "codex") as the pane container's accessibility label. Every session renders as a pane
    /// inside one shared window whose title does not encode which session is frontmost, so UI
    /// automation and VoiceOver identify the front window's selected-tab session by this label
    /// off the same `terminal-pane-<sessionID>` element that carries the owner/viewer AXValue.
    /// The panel coordinator keeps it current on every render pass.
