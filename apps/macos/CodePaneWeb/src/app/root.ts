...
export async function mountRoot(container: HTMLElement): Promise<CodePaneRootHandle> {
  ...
  const diffAutosaveHost: AutosaveHost = {
    ...
    blockedReason: () => {
      ...
      // See `lastCommitDiffEditorVerified`'s doc comment: a Last Commit draft is saveable only
      // once this page's immutable-revision read has proven the worktree still matches the
      // reviewed target, so this must block ahead of the conflict check below, before that read
      // resolves.
      if (state.scope.kind === "lastCommit" && !lastCommitDiffEditorVerified) return "Verifying Last Commit target";
  ...
  function openInEditor(path: string, options?: { line?: number }): void {
    navigationGeneration += 1;
    // `editorView.open` can refuse (a dirty buffer whose mandatory flush failed or is blocked) or
