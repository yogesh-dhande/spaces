...
export class EditorView {
  ...
  constructor(
    ...
    // collapsing it into its text. Live announcements stay off: the failed state re-renders its
    // retry countdown every second, which would otherwise be read aloud each tick.
    this.saveStatusChip.setAttribute("role", "status");
    this.saveStatusChip.setAttribute("aria-live", "off");
   ...
   *  line travels WITH that load rather than through a shared field on purpose: an open that has to
   *  wait on a flush can be superseded by a later one, and the superseded open must never redirect
   *  the winner's reveal to its own line. */
  async open(path: string, options?: { revealLine?: number }): Promise<void> {
