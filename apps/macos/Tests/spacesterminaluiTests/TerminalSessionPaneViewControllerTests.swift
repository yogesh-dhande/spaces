...
final class TerminalSessionPaneViewControllerTests: XCTestCase {
    ...
    private final class ClientCapture: @unchecked Sendable {
        ...
        /// Whether the close hook reported the pane as the session's owner.
        var closeHookHeldOwnership: Bool?
    }
    ...
    @MainActor func testCloseHookReportsOwnershipForOwnerPane() async throws {
        ...
        let controller = TerminalSessionPaneViewController(
            ...
            detachClientAction: { clientID in capture.detachedClientID = clientID },
            onCloseClientDetached: { heldOwnership in
                capture.closeHookHeldOwnership = heldOwnership
                hookRan.fulfill()
            })
        ...
        await fulfillment(of: [hookRan], timeout: 1)
        XCTAssertEqual(capture.closeHookHeldOwnership, true)
    }
    ...
    @MainActor func testCloseHookReportsNoOwnershipForViewerPane() async throws {
        ...
        await fulfillment(of: [hookRan], timeout: 1)
        XCTAssertEqual(capture.closeHookHeldOwnership, false)
    }
