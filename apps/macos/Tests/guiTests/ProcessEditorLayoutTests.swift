...
@MainActor
@Suite struct ProcessEditorLayoutTests {
    ...
    @Test func statusCheckRowShowsCommandField() {
        ...
        let checkCommandFields = findTextFields(in: editor.container, matching: { field in
            field.placeholderString == "command" && field.font?.pointSize == 11
        })
    ...
    private func findTextFields(in view: NSView, matching predicate: (NSTextField) -> Bool) -> [NSTextField] {
        ...
        if let field = view as? NSTextField, predicate(field) {
            results.append(field)
        }
        for subview in view.subviews {
            results.append(contentsOf: findTextFields(in: subview, matching: predicate))
        }
        return results
    }
}
