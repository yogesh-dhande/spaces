...
final class ProcessOnExitTests: XCTestCase {
    ...
    func testProcessOnExitNoneDoesNothing() throws {
        ...
        try store.setWorkspaceProcesses(workspaceID: workspace.id, processes: [
            ProcessTemplate(name: "api", command: "echo api", onExit: .none)
        ])
        
        let runningProcess = RunningProcessRecord(
            id: UUID().uuidString, workspaceID: workspace.id, templateName: "api", command: "echo api",
            terminalApp: nil, windowID: nil, pid: 9000, status: .running, logPath: nil,
        ...
        let updatedProcess = RunningProcessRecord(
            id: runningProcess.id, workspaceID: workspace.id, templateName: "api", command: "echo api",
            terminalApp: nil, windowID: nil, pid: 9000, status: .exited, logPath: nil,
            lastOutputAt: nil, startedAt: "now", exitedAt: "now")
    ...
    func testProcessOnExitNotifyShowsNotification() throws {
        ...
        try store.touchWorkspaceSettings(workspaceID: workspace.id, updatedAt: "now")
        try store.setWorkspaceProcesses(workspaceID: workspace.id, processes: [
            ProcessTemplate(name: "api", command: "echo api", onExit: .notify)
        ])
    ...
    func testProcessOnExitRestartRestartsProcess() throws {
        ...
        try store.touchWorkspaceSettings(workspaceID: workspace.id, updatedAt: "now")
        try store.setWorkspaceProcesses(workspaceID: workspace.id, processes: [
            ProcessTemplate(name: "api", command: "npm start", onExit: .restart)
        ])
        
        let runningProcess = RunningProcessRecord(
            id: UUID().uuidString, workspaceID: workspace.id, templateName: "api", command: "npm start",
            terminalApp: "iTerm2", windowID: 123, pid: 9000, status: .running, logPath: nil,
            lastOutputAt: nil, startedAt: "now", exitedAt: nil)
        try store.upsert(runningProcess: runningProcess)
