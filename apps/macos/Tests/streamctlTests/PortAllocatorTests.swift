...
final class PortAllocatorTests: XCTestCase {
    func testAllocateSkipsReservedPorts() throws {
        ...
        let ports = try allocator.allocatePorts(workspaceID: workspaceB.id, count: 2, range: PortRange(start: 20000, end: 20003))
    ...
    func testAllocateThrowsWhenInsufficientPorts() throws {
        ...
        XCTAssertThrowsError(try allocator.allocatePorts(workspaceID: workspaceB.id, count: 2, range: PortRange(start: 20000, end: 20002))) { error in
