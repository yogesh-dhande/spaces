...
@Suite struct AppKitControllerActivationSelectionTests {
    @Test func activationPrefersFocusedWorkspace() {
        #expect(WindowFocusController.activationSelectionTarget(focusedWorkspaceID: "workspace-focused") == .workspace("workspace-focused"))
    }

    @Test func activationKeepsCurrentViewWhenFocusedWindowIsNotTracked() {
        #expect(WindowFocusController.activationSelectionTarget(focusedWorkspaceID: nil) == nil)
    }
