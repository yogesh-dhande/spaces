...
@Suite struct ReferencedTerminalSessionIDsTests {
    private func overview(retained: [String], sessions: [String] = []) -> SpacesDeviceOverviewPayload {
        SpacesDeviceOverviewPayload(
            workspaces: [], sessions: sessions.map(sessionSummary(id:)), retainedTerminalSessionIDs: retained)
    }

    private func sessionSummary(id: String) -> SpacesDeviceTerminalSessionSummary {
        SpacesDeviceTerminalSessionSummary(
            id: id, title: "shell", workingDirectory: "/tmp/workspace", shell: "/bin/zsh", command: nil, state: .running,
            backend: .ghosttyEmbedded, lifetimePolicy: .persistent, servicePID: 1234, childPID: 5678, workspaceID: "workspace",
            workspaceTitle: "Workspace", projectID: "project", projectName: "Project", createdAt: "2026-07-20T00:00:00Z",
            updatedAt: "2026-07-20T00:00:01Z", isControlAvailable: true, isSubscriptionAvailable: true,
    ...
    @Test func keepSetIncludesRetainedSessionAbsentFromSessionsList() {
        ...
        let toClose = OpenPanePruning.sessionsToClose(
            openPanes: [OpenPanePruning.OpenPane(deviceID: "local", sessionID: "sess-ended-shell")], deviceID: "local",
            catalogSessionIDs: keepSet)
    ...
    @Test func keepSetExcludesSessionTheDaemonNoLongerRetains() {
        ...
        let toClose = OpenPanePruning.sessionsToClose(
            openPanes: [OpenPanePruning.OpenPane(deviceID: "local", sessionID: "sess-removed")], deviceID: "local",
            catalogSessionIDs: keepSet)
