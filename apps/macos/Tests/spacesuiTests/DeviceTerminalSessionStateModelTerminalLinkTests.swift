...
extension ProcessProfileEnvironmentSuites {
    ...
    ///
    /// Mutates the process-global profile environment: `handleResolveTerminalLinkRequest` opens the
    /// profile database to load workspace roots for every local-file link. Pinning both the database and
    /// runtime directory keeps that access off the developer's profile and gives this temporary profile
    /// its own daemon lock. Nests under `ProcessProfileEnvironmentSuites` (rather than declaring its own
    /// `.serialized`) for that reason; see that parent's doc comment for why a per-suite trait alone does
    /// not keep two such suites from clobbering each other's overrides. Relative-path anchoring is covered
    /// hermetically by the resolver's own unit tests in `spacescliTests`.
    @Suite final class DeviceTerminalSessionStateModelTerminalLinkTests {
                ...
                let response = try DeviceTerminalSessionStateModel.sendTerminalServiceRequest(
                    TerminalServiceRequest(
                        command: .resolveTerminalLink(
                            TerminalServiceTerminalLinkResolveRequest(
                                sessionID: "missing-session", terminalLink: "https://example.com/screenshot.png"))),
