...
@Suite struct RemoteTerminalWindowClientStoreTests {
    ...
    /// Each tick is driven directly (`sendHeartbeat`) rather than awaited from the real timer, so the
    /// tests are about the rule and never about how promptly a loaded machine schedules a global queue.
    @Test func heartbeatStopsOnceTheDaemonAnswersThatTheClientIsGone() {
        let counter = CallCounter()
        let store = TerminalPaneService.RemoteTerminalWindowClientStore(heartbeatAction: { _ in
            _ = counter.increment()
            return false
        })

        store.set("client")
        #expect(store.isHeartbeatArmed)
        store.sendHeartbeat()
        #expect(counter.current() == 1)
        #expect(!store.isHeartbeatArmed, "a heartbeat the daemon answers with 'gone' stops the timer")
        #expect(store.current() == "client", "the stored client id survives the stop so the next attach can re-arm")
    ...
    @Test func heartbeatKeepsGoingWhenTheAnswerArrivesAfterAReattach() {
        let counter = CallCounter()
        final class StoreBox: @unchecked Sendable { weak var store: TerminalPaneService.RemoteTerminalWindowClientStore? }
        let box = StoreBox()

        let store = TerminalPaneService.RemoteTerminalWindowClientStore(heartbeatAction: { clientID in
        ...
        store.sendHeartbeat()
        #expect(counter.current() == 1)
        #expect(store.isHeartbeatArmed, "a stale 'gone' answer that raced a re-attach must not strand the fresh attachment")
        store.sendHeartbeat()
