...
@MainActor @Suite(.serialized) struct CodePaneContentControllerTests {
    ...
    /// `close()` tears the web view down while the flush is still waiting for the page's answer. A
    /// torn-down page can never answer, and `close()`'s own fence (`closeLifecycleWorkIsSettled`)
    /// does not include `pendingEditsFlush`, so the completion must be settled by teardown itself
    /// rather than left for the flush's timeout: otherwise a controller that deallocates before the
    /// timeout fires drops the completion and `PanelCoordinator`'s termination `DispatchGroup` never
    /// leaves.
    @Test func closeSettlesAPendingEditsFlushImmediatelyRatherThanWaitingForTheTimeout() throws {
        let pane = try readyPaneWithRecordingEvaluator()
        var completions = 0

        pane.content.flushEditsBeforeTermination(timeout: 5) { completions += 1 }
        let token = try flushEditsToken(pane.evaluator)

        pane.content.close()

        #expect(completions == 1, "close() must settle the pending flush synchronously, not leave it for the 5s timeout")

        // The page's own answer, arriving after teardown, must not run the completion a second time.
        sendEditsFlushed(token: token, to: pane.content, from: pane.webView)
        #expect(completions == 1, "a page answer after teardown must not complete an already-settled flush again")

        // `close()` registered a state collector in the app-wide handoff registry; answer it so the
        // registry is empty again for the termination-fence tests that run after this one.
        pane.evaluator.completeOldestPending(with: try workspaceStateJSON(completeWorkspaceState()))
    }

    /// `deactivate()` (hibernation) shares the same teardown path as `close()`. Hibernation carries
    /// the unsaved buffer forward in the state snapshot, so settling the flush here rather than
    /// waiting for the page (which is gone) loses nothing.
    @Test func deactivateSettlesAPendingEditsFlushImmediatelyRatherThanWaitingForTheTimeout() throws {
        let pane = try readyPaneWithRecordingEvaluator()
        var completions = 0

        pane.content.flushEditsBeforeTermination(timeout: 5) { completions += 1 }
        _ = try flushEditsToken(pane.evaluator)

        pane.content.deactivate()

        #expect(completions == 1, "deactivate() must settle the pending flush synchronously, not leave it for the 5s timeout")
        pane.evaluator.completeOldestPending(with: try workspaceStateJSON(completeWorkspaceState()))
    }
