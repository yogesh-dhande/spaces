...
final class TerminalEngineInputUnderOutputLoadRegressionTests: XCTestCase {
    ...
    /// Points the profile database at a per-suite temp root so the session core's durable writes hit an
    /// isolated database, never the user's real profile database (which may also be schema-newer or
    /// daemon-owned, refusing to open).
