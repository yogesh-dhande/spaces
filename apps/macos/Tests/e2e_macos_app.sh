...
source "$MACOS_DIR/Tests/e2e_ui_automation.sh"
...
-- Pierre only renders a window of a long document. Which window that is moves during a paste (the
-- caret lands at the end, then the autosave's refresh restores the viewport line), so a paste is
-- recognised by any of its non-empty lines rather than by the whole string or a fixed line. Every
-- caller replaces the buffer with content sharing no line with the prior text, which the
-- pre-paste check relies on.
...
on nonEmptyLines(targetValue)
  ...
  repeat with rawLine in rawLines
    if (rawLine as text) is not "" then set end of kept to (rawLine as text)
  end repeat
  if (count of kept) is 0 then error "editor replacement value is empty"
  return kept
end nonEmptyLines

on containsAnyLine(reported, targetLines)
  repeat with targetLine in targetLines
    if reported contains (targetLine as text) then return true
  end repeat
  return false
end containsAnyLine
...
on run argv
  ...
  try
    ...
    set targetLines to my nonEmptyLines(targetValue)
    ...
    if my containsAnyLine(my editorReportedValue(editorElement), targetLines) then
    ...
    repeat while (current date) < pasteDeadline
      ...
      if currentElement is not missing value then
        ...
        if my containsAnyLine(reported, targetLines) then
          set pasted to true
          exit repeat
        end if
      end if
