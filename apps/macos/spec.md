...
- App-launched built-in workspace terminals, process windows, and coding-agent windows should use the live Ghostty renderer path immediately and reopen onto the same local session while `SpacesApp` remains alive.
...
- The terminal service should start the first-party mobile bridge on launch, keep the stable default port reserved while it is running, advertise the bridge to nearby iOS clients with Bonjour, and retry bridge startup when the configured port becomes available.
- The Mac sidebar should expose a mobile connection action next to user settings that shows the bridge pairing code, port, Bonjour name, and reachable addresses.
...
- Native Spaces terminal windows should attach to an existing session without restarting the shell, preferring the live in-process Ghostty host when the app already owns that session and otherwise subscribing to the daemon-owned session state stream for owner handoff compatibility, final Ghostty renders, and metadata synchronization.
...
- The iOS client should discover nearby Mac bridges, authenticate once with a pairing code from the Mac bridge, store the issued credential, reconnect without prompting on later launches, reject non-first-party bundle identities, and clear the stored credential with a re-pair prompt if the Mac later rejects that device token.
- The iOS client should be able to browse all live terminal sessions, auto-attempt ownership takeover when opening one, render one session at a time through a Ghostty-backed terminal surface only after ownership is acquired, and keep ended-session final renders available when Ghostty state was persisted.
...
- The iOS terminal keyboard accessory should use a compact fixed-height toolbar: `/` and `~` lead the scrollable key strip, while a single arrow-key joystick and the keyboard hide/show control stay pinned on the trailing edge.
...
- Live non-owner terminal windows on macOS and iOS should show takeover and status UI instead of terminal content. If the session has already ended and a final Ghostty render is available, that final render may still be shown with terminal-ended chrome.
