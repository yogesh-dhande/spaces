...
- The iOS terminal keyboard accessory should use a compact fixed-height toolbar: `/` and `~` lead the scrollable key strip, iPhone uses tighter button widths and spacing than iPad, and a single arrow-key joystick plus the keyboard hide/show control stay pinned on the trailing edge.
