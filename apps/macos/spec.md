...
- Built-in `script-pty` sessions should also answer common terminal identity and color queries from TUI clients such as `codex`, so prompt and status rows do not stall waiting for `CSI 6n`, device-attribute, or terminal-color replies that a real terminal would normally provide.
