...
- `script-pty` terminal sessions should open in a native Spaces window that preserves the underlying shell session across Spaces app restarts, shows recent output, and accepts direct keyboard input, paste, and special keys through the session control socket without relying on a separate send-input form.
- `script-pty` terminal sessions should also be exposable through `spaces terminal proxy`, which forwards the same owner/viewer host protocol over TCP behind a shared auth token for mobile-shaped or remote clients.
- The active owner client for a built-in `script-pty` terminal should also propagate live terminal geometry so full-screen TUIs, wrapping, and alternate-screen apps track the current window size instead of staying stuck on launch-time dimensions.
- Built-in `script-pty` sessions should also start with a sane non-zero terminal size before the first client window attaches, so full-screen TUIs such as `codex` do not paint a broken first frame while waiting for the first live resize from the owner window.
- Built-in `script-pty` sessions should also answer common terminal identity, mode-report, and color queries from TUI clients such as `codex`, so prompt and status rows do not stall waiting for cursor-position, device-attribute, private-mode, or terminal/palette-color replies that a real terminal would normally provide.
- Built-in `script-pty` sessions should also answer terminal-size queries from the PTY host's current geometry, so advanced TUIs that probe window size through `CSI 18 t` see the same rows and columns the owner client negotiated.
- Built-in `script-pty` sessions should also answer xterm title-report queries (`CSI 20 t`, `CSI 21 t`) from the same observed `OSC 0`, `OSC 1`, and `OSC 2` state that drives shared terminal window titles.
- The shared-session `script-pty` terminal surface should preserve common ANSI styling and cursor state in the local renderer, including palette colors, truecolor, bold/italic/underline/inverse text, cursor visibility, and alternate-screen enter and exit.
- The shared-session `script-pty` terminal surface should also honor xterm cursor-style updates, so block, underline, and bar cursors render distinctly instead of collapsing to one fallback shape.
- The shared-session `script-pty` terminal surface should also honor terminal-driven title updates (`OSC 0`, `OSC 1`, `OSC 2`), so built-in terminal windows reflect shell and TUI title changes instead of staying pinned to the launch title.
- The shared-session `script-pty` terminal surface should also honor terminal-driven working-directory updates (`OSC 7`), so built-in terminal metadata and represented paths follow shell directory changes instead of staying pinned to the launch directory.
- When a shared-session terminal enters and exits an alternate-screen app, the underlying primary screen should recover its saved cursor and scroll-region state instead of only restoring visible rows.
- The shared-session `script-pty` terminal surface should also honor xterm private cursor-save mode `?1048`, so TUIs that save cursor state independently of alternate-screen swaps restore to the expected position.
- The shared-session `script-pty` terminal surface should also preserve primary-screen scrollback when full-screen primary-screen apps scroll the whole terminal region, so users can scroll back through prior output even when the active viewport is being redrawn in place.
- The shared-session `script-pty` terminal surface should restore a user-scrolled viewport by visible content instead of only bottom offset, so large output batches do not jump the view to a different part of history.
- When a user is actively browsing shared-session scrollback away from the bottom, passive refresh should back off the high-churn cadence so incoming output does not visibly fight manual reading or scrolling.
- The shared-session `script-pty` terminal surface should also forward standard terminal mouse input when the active screen enables mouse reporting, including scroll-wheel events for full-screen TUIs while preserving ordinary local selection and scrollback browsing when mouse reporting is off.
- The shared-session `script-pty` terminal surface should honor bracketed paste mode when the active screen enables `DECSET 2004`, so clipboard paste behaves like a real terminal instead of sending raw pasted text.
- The shared-session `script-pty` terminal surface should also honor `DECSET 1004` focus reporting and IME composition, so focus-aware TUIs and non-Latin text entry use the same owner-input path as ordinary typing.
- When the shared-session renderer captures `OSC 8` hyperlinks, command-click on the rendered link text should open the target instead of treating it as inert decoration.
- The shared-session `script-pty` terminal surface should also forward classic xterm mouse-reporting sequences when the active screen enables legacy mouse modes without `1006`, and it should expose extended function-key, modifier, and keypad combinations that modern TUIs expect from a local terminal.
- When the shared-session `script-pty` renderer enters or exits an alternate-screen app, it should treat that as a screen swap rather than ordinary appended output: stale text selection should clear, preserved horizontal scroll should reset, and the viewport should snap to the active full-screen frame instead of trying to keep the previous scrollback position.
...
- Built-in `Spaces` terminal, process, and coding-agent launches should use the same persistent session-host model so closing and reopening the app detaches and reattaches clients without restarting the underlying shell session.
...
- `spaces terminal takeover` should transfer input authority to the selected viewer attachment without restarting the session, and later attached clients should observe the updated owner or viewer state from persisted attachment records.
