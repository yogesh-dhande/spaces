...
- `GhosttyEmbeddedSessionHost` in `spacesterminalghostty` is the first session host layer for `ghostty-embedded`. It owns one libghostty-backed session runtime, the local control socket, output logging, runtime-state refresh, the active owner client ID for that session, host-stamped remote lease timestamps, and lease expiry for stale remote clients.
...
- The current native window keeps one shared window-controller shell around the libghostty session host. Active owner windows keep the live libghostty surface. Passive viewer windows stay in a loading state until the live libghostty surface can be read back in-process, and they switch to persisted final output only after the session exits.
...
- `TerminalSessionWindowController` does not permanently trust the metadata snapshot it observed during construction. If `metadata.json` becomes readable only after the controller exists, the next refresh upgrades the controller to the persisted renderer mode instead of leaving that window stuck on fallback tail chrome for the rest of its lifetime.
...
- `AppKitController` also rechecks ad hoc session teardown on attachment-state changes when no native controller remains, so remote or mobile viewers can keep a session alive after the last local window closes without leaking it after the final live attachment detaches or expires.
...
- `spacesterminalcore` reconstructs `spaces terminal tail` output from `output.log` with `libghostty-vt` instead of the older custom ANSI transcript renderer. The live Ghostty owner view still owns the PTY and in-process surface; the VT library is used only for non-surface replay from persisted bytes.
- `spacesterminalcore` also exposes a small stateful `libghostty-vt` bridge that can hold terminal state independently of a live Ghostty surface, accept incremental VT byte streams, and export viewport snapshots plus terminal metadata for future split-ownership experiments.
- `GhosttyEmbeddedTerminalView` reports live terminal cell-size changes back into the session host, and the host persists those columns and rows in `state.json` whenever the owner window is resized so replay consumers can stay aligned with the last visible screen geometry.
- The app-hosted `ghostty-embedded` path keeps `control.sock`, `output.log`, `clients.json`, and `attachments.json` as the session boundary so later owner or viewer attachment can fan out beyond the local macOS window instead of baking control into one view. The current mobile-shaped path uses `spaces terminal proxy` as a thin TCP bridge over that boundary rather than introducing a second terminal protocol.
- `spaces terminal proxy` is a transitional transport primitive, not the intended long-term external product interface. It exists to prove remote-shaped attach, tail, send, key, and takeover behavior against the Ghostty-owned session boundary. A broader host service can later subsume that per-session bridge while reusing the same ownership model and session files underneath.
...
- Owner or viewer state is persisted in `attachments.json`. Local attachments stay explicit, while remote attachments are treated as leases that must be refreshed by client-identified proxy or control traffic and are stamped from host time when the attach lands. CLI `send` and `key` remain privileged control-plane commands, while attached UI clients identify themselves so the session host can reject input from passive viewer windows and accept ownership transfer through `spaces terminal takeover`.
- The current terminal slice is Ghostty-owned: built-in sessions are created only when Ghostty artifacts are installed locally, and passive/mobile clients attach through the same control socket, output log, and attachment records that back the macOS owner window.
...
- Built-in process and agent focus prefer the live native window when a tracked yabai window ID exists, then use the in-process terminal controller focus IPC for owner-surface focus, and only reopen the session when no live native window can be focused.
...
- The built-in terminal runtime must keep raw PTY output, exact input injection, and current terminal geometry available at the session boundary so the CLI, passive viewers, and future remote clients can share the same control model.
...
- Live libghostty state stays in-process inside the app, while persisted session output plus geometry provide the replay source for `spaces terminal tail` and post-exit inspection.
