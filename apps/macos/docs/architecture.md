...
- `spacescli`: declarative `swift-argument-parser` command tree for `spaces`, including command help, leaf validation, translation from CLI inputs into orchestration calls, the built-in `spaces terminal` session controls for `command`, `send`, `key`, `tail`, `list`, `show`, and `takeover`, the `spaces mobile` status and standalone bridge controls, and profile or desktop-control inspection helpers used by dev and real-system workflows.
...
- `spacesmobilebridge`: daemon and CLI-hosted first-party TCP bridge for the iOS client, including stable bridge settings, pairing persistence, workspace or session overview shaping, Bonjour advertisement, and bridge restart supervision.
...
- The current Ghostty fork still couples PTY ownership to a libghostty renderer. Because of that, the service keeps the live hidden Ghostty host and publishes one full terminal-grid snapshot for owner bootstrap instead of attaching multiple native renderers to the same session.
- Mobile owner bootstrap uses a Ghostty session snapshot captured while the Mac local window owns the terminal. The Mac host refreshes that cached snapshot immediately before transferring ownership to a remote mobile owner, without forcing a Mac-side Ghostty surface refresh, so iOS receives the current visual surface instead of a stale input/output resync snapshot. It must not use the VT snapshot stream as an owner-bootstrap source because VT reconstruction does not preserve the same visual surface state. Owner resync, resize, and live-output rendering do not call `ghostty_session_export_snapshot`.
- Passive viewer open and reopen do not export a live Ghostty session snapshot. When the active owner is a local Mac window, `reason=initial` publishes metadata, runtime state, and attachment state only; when the active owner is a remote mobile owner, `reason=initial` publishes the cached session snapshot for the owner epoch.
...
- The current iOS renderer bridge feeds the owner bootstrap snapshot into a local Ghostty renderer session, applies streamed output bytes after the owner epoch is established, and keeps keyboard ownership outside the renderer so the transport path survives a future direct session or delta import API.
- Mobile owner rendering has one live bootstrap source: the Mac or service owner publishes a Ghostty session snapshot, and the iOS owner replays that snapshot into its local Ghostty surface for the owner epoch. After that epoch is established, iOS does not call `ghostty_session_export_snapshot` on its local renderer for live-output rendering or E2E render-dump capture.
- The iOS renderer reports the local Ghostty surface geometry back to the Mac bridge for owner resizing. On iPhone it caps the reported columns to the readable inset surface width so the remote session does not keep emitting columns that render beyond the visible phone edge; iPad keeps the raw Ghostty column count.
- iOS render-dump text is limited to the owner bootstrap snapshot or status text. Full output-history is stored only as a lazy scrollback seed for the local Ghostty surface and is not re-rendered through a separate text path, so E2E checks use readiness, event logs, screenshots, and performance events for live-output and scrollback coverage.
- `SpacesMobileBridgeServer` is the current first-party remote transport seam. `SpacesMobileBridgeSupervisor` starts it with profile-scoped settings, advertises `_spaces-mobile._tcp.` through Bonjour, and retries startup while leaving a conflicting port untouched. The transport is intentionally scoped to the Spaces iOS client rather than a third-party-stable public API.
- `AppKitController` reads `SpacesMobileBridgeSettingsStore` for the sidebar mobile connection action, so the Mac UI and `spaces mobile status` present the same profile-scoped pairing details.
