...
- `spaces terminal proxy` binds a local TCP listener on top of that same session boundary. It reuses the control request/response codec, requires a shared auth token, answers `attach` and `tail` directly from persisted session state, and forwards the rest of the control plane back into `control.sock`.
