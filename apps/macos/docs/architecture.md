...
- `TerminalSessionWindowController` chooses between the app-owned live host and `RemoteGhosttySessionHost` by session ID. When `SpacesApp` already owns that session locally it reattaches the live Ghostty surface; otherwise it follows the service-owned control plane and consumes the daemon snapshot stream for passive renderer updates, falling back to `output.log` plus `state.json` replay with `libghostty-vt` only when the live stream is unavailable.
