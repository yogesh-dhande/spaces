...
- `TerminalScreenBuffer` also preserves terminal-driven working-directory state from `OSC 7`, and the shared window-controller shell uses that buffered path for `script-pty` represented URLs and summary metadata so transport-backed sessions follow the shell's active directory instead of only the launch configuration.
