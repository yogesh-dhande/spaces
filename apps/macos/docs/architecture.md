...
- Mobile owner bootstrap uses a cached Ghostty session snapshot captured while the Mac local window owns the terminal. The Mac host refreshes that cached session snapshot immediately before transferring ownership to a remote mobile owner, so the iOS bootstrap does not reuse a stale input/output resync snapshot containing transient line-editor cells. It must not use the VT snapshot stream as an owner-bootstrap source because VT reconstruction does not preserve the same visual surface state. Owner resync, resize, and live-output rendering do not call `ghostty_session_export_snapshot`.
