...
- `ScriptPTYTerminalSessionRuntime` also answers a small set of terminal queries directly from the PTY host boundary. The current responder covers cursor-position, primary/secondary device attributes, tracked private-mode reports, live terminal-size reports, terminal foreground/background color queries, and palette-color probes so TUI startup handshakes can complete even when no libghostty surface is attached to the session host.
