...
- `apps/macos/Tests/e2e_terminal_mobile_client.sh` is the repository-level regression harness for that path. It drives an isolated app instance through the TCP proxy seam and verifies that owner attach, viewer attach, takeover, text input, and key input all stay wired to the same Ghostty-owned session.
