...
- The current Ghostty fork still couples PTY ownership to a libghostty renderer. Because of that, the service keeps the live hidden Ghostty host and treats `output.log` plus `state.json` as the first shared rendering surface for other clients, with passive macOS clients rebuilding a local VT snapshot from those persisted files.
- `TerminalSessionWindowController` chooses between the app-owned live host and `RemoteGhosttySessionHost` by session ID. When `SpacesApp` already owns that session locally it reattaches the live Ghostty surface; otherwise it follows the service-owned control plane and rebuilds passive renderer snapshots from `output.log` plus `state.json` with `libghostty-vt`.
...
- `spacesterminalcore` reconstructs `spaces terminal tail` output from `output.log` with `libghostty-vt`. It also exposes a stateful VT bridge that can hold terminal state independently of a live Ghostty renderer and is the intended bootstrap path for future snapshot-plus-delta clients.
