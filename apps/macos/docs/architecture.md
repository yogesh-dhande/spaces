...
- Built-in `Spaces` process and terminal focus prefers the tracked live yabai window ID when one exists, falls back to reopening the session by stable session ID only when no live native window can be focused, and clears stale native window bindings immediately after a reopen so normal workspace focus stops targeting the dead window before the next refresh pass.
