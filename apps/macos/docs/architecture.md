...
- The current native window keeps one shared window-controller shell but swaps its body by backend: `script-pty` uses a transport-backed transcript client that renders recent output and sends direct keyboard input, paste, and named keys through the session transport, while `ghostty-embedded` keeps the active owner window on the session host's libghostty surface. Passive viewer windows stay read-only and prefer a rendered snapshot of that shared libghostty surface; they fall back to tailed output only when no inspectable surface is available in-process.
