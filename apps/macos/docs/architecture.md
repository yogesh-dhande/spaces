...
- The current native window keeps one shared window-controller shell around the libghostty session host. Active owner windows keep the live libghostty surface, while passive viewer or unavailable-snapshot states fall back to tailed output when the installed GhosttyKit build does not expose an inspectable surface API in-process.
