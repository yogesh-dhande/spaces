...
- Remote screen state prefers a live Ghostty surface export, but the service falls back to VT reconstruction from `output.log` when that export is temporarily empty so the first mobile-owner frame is still populated during takeover.
