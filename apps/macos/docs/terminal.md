...
- the same `TerminalScreenBuffer` also keeps primary-screen scrollback when a full-screen primary-screen app scrolls the whole terminal region, so Codex-style redraws can keep prior output available above the current viewport instead of discarding it as soon as it scrolls off the top row
- the shared-session canvas also tracks bracketed paste and alternate scroll mode, so TUIs that enable `DECSET 2004` or `DECSET 1007` keep terminal-native paste and wheel behavior instead of receiving raw clipboard text or inert scroll events
...
- `implemented`: terminal-driven working-directory updates (`OSC 7`) propagate through the shared renderer so represented paths and metadata follow shell directory changes
...
- `implemented`: modern xterm styling coverage in the shared renderer includes faint, italic, hidden, strikethrough, double underline, underline color, and the common reset semantics exercised by modern shells and TUIs
