    ...
    @MainActor final class SpacesMobileAutomationsTests: XCTestCase {
        ...
        func testRowsSortByNameAndCarryLastRunStatus() {
            let automations = [
                makeAutomation(id: "automation-b", name: "Nightly backup"),
                makeAutomation(id: "automation-a", name: "Deploy"),
            ]
            let runs = [
        ...
        func testRunRowsFilterByAutomationAndSortNewestFirst() {
            ...
            XCTAssertEqual(
                SpacesMobileAutomations.runRows(runs, automationID: "automation-a").map(\.id), ["run-a-new", "run-a-old"])
        ...
        func testTriggerSummaryDistinguishesManualAndCron() {
            ...
            XCTAssertEqual(SpacesMobileAutomations.triggerSummary(makeAutomation(triggerKind: "cron", cronExpression: "*/5 * * * *")), "Cron: */5 * * * *")
        }
        ...
        func testNextFireDescriptionOnlyForEnabledCronWithFireTime() {
            ...
            XCTAssertNil(
                SpacesMobileAutomations.nextFireDescription(makeAutomation(enabled: false, nextFireTime: iso), relativeTo: now))
        ...
        func testAlertEntriesOnlyIncludeFailedAndTimedOutNewestFirst() {
            let runs = [
                ...
                makeRun(id: "run-failed", automationID: "a", automationName: "Deploy", status: "failed", exitCode: 3, endedAt: "2026-01-01T00:00:00Z"),
        ...
        func testUndismissedAlertCountAndClearIncludeAutomationAlerts() {
            let model = makeModel()
            model.overview = makeOverview(
                automationRuns: [makeRun(id: "run-failed", automationID: "a", automationName: "Deploy", status: "failed")])
        ...
        func testTriggerAutomationSendsTriggerThenReloadsOverview() async {
            ...
            let refreshedOverview = makeOverview(
                automations: [makeAutomation(id: "automation-a", name: "Deploy")], automationRuns: [triggeredRun])
        ...
        func testCancelAutomationRunSendsCancelThenReloadsOverview() async {
            ...
            let refreshedOverview = makeOverview(
                automations: [makeAutomation(id: "automation-a", name: "Deploy")], automationRuns: [canceledRun])
            let client = SpacesDeviceAPIClient(settings: settings) { request in
                await recorder.append(request)
