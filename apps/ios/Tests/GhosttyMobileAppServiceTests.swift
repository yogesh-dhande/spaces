    ...
    @MainActor final class GhosttyMobileAppServiceTests: XCTestCase {
        ...
        func testRemoteTerminalHostViewTapOnLinkConsumesTapBeforeFocus() {
            ...
            hostView.debugAppliedFrameCoversHostGridForTesting = true
        ...
        /// While the phone shows only a slice of the host's grid (every frame between taking ownership and
        /// the owner runtime resizing to the phone's width), a tap must not activate a link at all. The
        /// probe joins the rows the frame marks as soft-wrapped, and those marks survive a column crop
        /// while the columns past the phone's width do not, so probing there yields a link the user never
        /// saw (#492). The tap falls through to the ordinary handling instead, and the same tap once the
        /// frame is whole opens the link.
        func testRemoteTerminalHostViewTapDoesNotProbeForLinksWhileTheFrameIsCropped() {
            ...
            XCTAssertFalse(hostView.debugAppliedFrameCoversHostGridForTesting, "a view that has applied no frame must not be treated as showing one")
            XCTAssertEqual(hostView.debugTapToActivateInputForTesting(at: CGPoint(x: 12, y: 18)), .focused)
            XCTAssertEqual(probeCount, 0, "a cropped frame must not be probed for links")
        ...
        /// track the host grid the phone is cropping: a Mac-width snapshot in a phone-width viewport is a
        /// slice, and the same snapshot at the viewport's own size is the grid whole.
        func testRemoteTerminalHostViewRecordsWhetherTheAppliedFrameCoversTheHostGrid() throws {
            GhosttyRemoteTerminalHostView.nativeMirrorEnabledForTesting = true
            let window = UIWindow(frame: UIScreen.main.bounds)
            let viewController = UIViewController()
            window.rootViewController = viewController
            ...
            XCTAssertFalse(
                hostView.debugAppliedFrameCoversHostGridForTesting, "a 105-column host grid in a 49-column viewport reaches the surface as a slice")

            hostView.update(snapshot: filledSnapshot(columns: 49, rows: 20), renderStateKey: "owner|phone-width", fallbackText: "")
            XCTAssertTrue(hostView.debugAppliedFrameCoversHostGridForTesting, "a host grid the viewport fits reaches the surface whole")

            hostView.prepareForDismantle()
            XCTAssertFalse(hostView.debugAppliedFrameCoversHostGridForTesting, "releasing the mirror drops what it recorded about the surface")
        }
