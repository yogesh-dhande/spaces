...
struct AlertsTabView: View {
    ...
    var body: some View {
            ...
            content
                ...
                .toolbar {
                    ToolbarItem(placement: .topBarTrailing) {
                        ...
                        .disabled(model.undismissedAlertCount == 0)
        ...
        .overviewPolling(model: model, tab: .alerts, activeDetailRouteID: selectedSession?.id)
    }
        ...
        if model.attentionGroups.isEmpty {
            ...
            ScrollView {
                LazyVStack(spacing: 0) {
                    ForEach(model.attentionGroups) { group in
                        alertGroupSection(group)
                            .padding(.bottom, 14)
                    }
                }
                .padding(.vertical, 12)
    ...
    @ViewBuilder private func eventRow(_ event: SpacesMobileAttentionEvent) -> some View {
        let row = BandRow(
            dotKind: StatusDot.Kind(attentionKind: event.kind), tile: .tile(for: event.rowType), title: event.title,
            detail: event.kind.label, detailIsMonospaced: false
        ) {
            Text(SpacesMobileAttention.abbreviatedAge(of: event.date))
                .font(.system(size: 11))
                .foregroundStyle(Theme.mutedSecondary)
                .monospacedDigit()
        }
        if let session = event.sessionID.flatMap({ model.session(forSessionID: $0) }) {
            ...
            .accessibilityIdentifier("alert.row.\(event.id)")
        } else {
            row
                .accessibilityIdentifier("alert.row.\(event.id)")
        }
