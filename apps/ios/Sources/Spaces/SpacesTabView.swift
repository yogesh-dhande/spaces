...
struct SpacesTabView: View {
    ...
    var body: some View {
            ...
            homeView
                ...
                .navigationDestination(item: $selectedBrowserSession) { route in
                    BrowserSessionDetailView(
                        title: route.row.title,
                        subtitle: route.row.route.identityHost,
                        request: route.proxyRequest,
        ...
        .accessibilityIdentifier("tab.spaces")
        .overviewPolling(
            model: model, tab: .spaces, activeDetailRouteID: activeDetailRouteID,
            refreshGeneration: terminalListRefreshGeneration)
    ...
    @ToolbarContentBuilder private var toolbarContent: some ToolbarContent {
        ToolbarItem(placement: .topBarTrailing) {
            if model.settings.isPaired {
                ...
                .disabled(model.isMutating || model.isActiveDeviceBlocked)
                .accessibilityIdentifier("spaces.newWorkspace")
            }
        }
    }
    ...
    private var searchFilterRow: some View {
        HStack(spacing: 8) {
            HStack(spacing: 7) {
                ...
                TextField("Search workspaces", text: $model.searchText)
                    .font(.system(size: 13))
    ...
    private func projectSection(_ projectGroup: ProjectGroup) -> some View {
        VStack(spacing: 0) {
            Text(projectGroup.projectName.uppercased())
                ...
                .padding(.horizontal, 20)
                .padding(.bottom, 8)
            ForEach(projectGroup.workspaceGroups) { group in
        ...
        VStack(spacing: 0) {
            Button {
                withAnimation(.easeInOut(duration: 0.2)) {
                    model.toggleWorkspaceCollapsed(group.id)
                }
            } label: {
                HeaderBand {
    ...
    @ViewBuilder private func runtimeTrailingIndicator(for row: SpacesMobileWorkspaceRuntimeRow) -> some View {
        if row.isBrowserSession || row.sessionID != nil {
            RowChevron()
        } else if row.canRun {
            RowPlayIndicator()
        }
...
struct WorkspaceCreateSheet: View {
    ...
    private var projects: [SpacesDeviceProjectSummary] {
        model.workspaceCreateOptions?.projects ?? model.overview?.projects ?? []
    }
        ...
        NavigationStack {
            ...
            .task {
                ...
                if selectedProjectID == nil {
                    selectedProjectID = model.workspaceCreateOptions?.selectedProjectID ?? projects.first?.id
                }
            }
        }
    }
