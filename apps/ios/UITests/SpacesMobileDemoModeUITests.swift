...
final class SpacesMobileDemoModeUITests: XCTestCase {
    ...
    func testDemoModeTourWithNoDaemon() throws {
        ...
        XCTAssertTrue(
            SpacesMobileUITestDriver.waitForText(containing: waitingAgentTitle, in: app, timeout: 20), "The waiting-agent alert was not visible")
        XCTAssertTrue(
            SpacesMobileUITestDriver.waitForText(containing: "Waiting for input", in: app, timeout: 10),
            "The waiting-for-input event kind was not visible")
        XCTAssertTrue(
        ...
        for _ in 0..<3 {
            ...
            if SpacesMobileUITestDriver.waitForDisappearance(of: demoBanner(in: app), timeout: 5) {
                demoModeCleared = true
                break
            }
        }
