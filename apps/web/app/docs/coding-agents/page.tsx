  ...
  return (
    ...
    >
      ...
      <article className="border-t border-line/70 pt-8 first:border-t-0 first:pt-0">
        ...
        <ul className="mt-3 space-y-2 text-sm leading-7 text-foreground-soft">
          <li>• <strong>Start one that&apos;s ready.</strong> <code>spaces agent spawn --command claude</code> opens a supported agent (Claude Code, Codex, or opencode) in a new workspace terminal and waits until it is ready before returning, so a follow-up <code>--prompt</code> lands reliably. It only starts agents whose Spaces hooks are installed.</li>
          ...
          <li>• <strong>Steer or stop it.</strong> <code>spaces agent interrupt</code> nudges an agent (sends Escape), and <code>spaces agent kill</code> ends it and its terminal.</li>
