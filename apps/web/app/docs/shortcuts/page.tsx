...
const shortcutRows = [
  { shortcut: "cmd+shift+=", action: "Toggle/focus Spaceship window", scope: "Global" },
...
export default function ShortcutsDocsPage() {
  return (
    <DocsShell
      title="Keyboard Shortcuts"
      description="Spaceship is designed for keyboard-first context switching between running workspaces and captured windows."
    ...
    >
      ...
      <article className="rounded-2xl border border-line bg-surface/82 p-5 backdrop-blur-sm">
        ...
        <ul className="mt-3 space-y-2 text-sm leading-7 text-foreground-soft">
          ...
          <li>• When Spaceship is not focused, next/previous cycles windows for the active or focused workspace.</li>
      ...
      <article className="rounded-2xl border border-line bg-surface/82 p-5 backdrop-blur-sm">
        ...
        <pre className="mt-3 w-full max-w-full min-w-0 overflow-x-auto whitespace-pre-wrap break-words rounded-xl border border-line bg-background-soft/80 p-3 text-xs leading-6 text-foreground">
          <code>{`spaceship settings get --gui-hotkey
spaceship settings set --gui-hotkey cmd+shift+9
spaceship settings set --gui-next-shortcut cmd+shift+]
spaceship settings set --gui-prev-shortcut cmd+shift+[
spaceship settings reset --gui-hotkey`}</code>
        </pre>
      </article>
      ...
      <article className="rounded-2xl border border-line bg-surface/82 p-5 backdrop-blur-sm">
        ...
        <p className="mt-2 text-sm leading-7 text-foreground-soft">
          When a text field is focused, Spaceship does not intercept standard text-edit shortcuts
